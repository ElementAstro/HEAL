...
class AsyncFileManager:
    ...
    def __init__(self, max_concurrent_operations: int = 10) -> None:
    ...
    async def read_file_async(
        ...
        async with self.semaphore:
            try:
                ...
                self.logger.debug(
                    f"异步读取文件完成: {file_path} ({execution_time:.3f}s)"
                )
    ...
    async def write_file_async(
        ...
        async with self.semaphore:
            try:
                ...
                execution_time = time.perf_counter() - start_time
                self.logger.debug(
                    f"异步写入文件完成: {file_path} ({execution_time:.3f}s)"
                )
    ...
    async def read_json_async(self, file_path: Union[str, Path]) -> AsyncIOResult:
        ...
        try:
            if result.data is None:
                return AsyncIOResult(
                    success=False,
                    error="No data to parse as JSON",
    ...
    async def write_json_async(
        ...
        try:
            ...
            return AsyncIOResult(
                success=False, error=f"JSON序列化失败: {e}", operation_type="json_write"
            )
    ...
    async def batch_read_files(
        ...
        # 处理异常结果
        processed_results: List[AsyncIOResult] = []
        for i, result in enumerate(results):
            ...
            elif isinstance(result, AsyncIOResult):
...
class AsyncNetworkManager:
    ...
    def __init__(self, max_concurrent_requests: int = 10, timeout: int = 30) -> None:
            ...
            try:
                async with aiohttp.ClientSession(timeout=self.timeout) as session:
                    async with session.get(
                        ...
                        execution_time = time.perf_counter() - start_time
                        self.logger.debug(
                            f"异步GET请求完成: {url} ({execution_time:.3f}s)"
                        )
            ...
            try:
                async with aiohttp.ClientSession(timeout=self.timeout) as session:
                    async with session.post(
                        ...
                        execution_time = time.perf_counter() - start_time
                        self.logger.debug(
                            f"异步POST请求完成: {url} ({execution_time:.3f}s)"
                        )
    ...
    async def batch_requests(
        ...
        for req in requests:
            ...
            if method == "GET":
                task = asyncio.create_task(self.get_async(
                    url, headers=req.get("headers"), params=req.get("params")
                ))
            elif method == "POST":
                task = asyncio.create_task(self.post_async(
                    url,
                    data=req.get("data"),
                ...
                # 不支持的方法，创建错误结果
                async def create_error_result() -> AsyncIOResult:
                    return AsyncIOResult(
                        success=False,
                ...
                tasks.append(asyncio.create_task(create_error_result()))
                continue
...
class AsyncIOWorker(QThread):
    ...
    def __init__(self, parent: Optional[QObject] = None) -> None:
        ...
        # 操作队列
        self.operation_queue: asyncio.Queue[Dict[str, Any]] = asyncio.Queue()
        self.running = False
    ...
    async def _execute_operation(self, operation: Dict[str, Any]) -> AsyncIOResult:
        ...
        if op_type == "read_file":
            return await self.file_manager.read_file_async(
                operation["file_path"], operation.get("encoding", "utf-8")
            )
            ...
            return await self.file_manager.write_json_async(
                operation["file_path"], operation["data"]
            )
...
async def write_file_async(
    ...
    """异步写入文件的便捷函数"""
    return await global_async_file_manager.write_file_async(

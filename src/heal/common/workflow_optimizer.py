...
class WorkflowOptimizer:
    ...
    def _resolve_dependencies(self) -> List[str]:
        ...
        # 拓扑排序
        queue = deque([name for name in self.steps.keys() if in_degree[name] == 0])
        result = []
    ...
    def execute(self, skip_optional_on_failure: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"开始执行工作流 {self.name}，共 {len(self.execution_order)} 个步骤")
    ...
    def _execute_step(self, step: WorkflowStep) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"步骤 {step.name} 执行成功，耗时 {step.execution_time:.3f}s")
            return True

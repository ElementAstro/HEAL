...
class OptimizationValidator:
    ...
    def run_comprehensive_validation(self) -> Dict[str, Any]:
        ...
        for name, benchmark in self.benchmarks.items():
            # 验证内存使用
            memory_result = self.validate_memory_usage(
                f"{name}_memory", benchmark)
            validation_summary["total_tests"] += 1
            if memory_result.passed:
                validation_summary["passed_tests"] += 1
            else:
                validation_summary["failed_tests"] += 1
                validation_summary["issues_found"].append(
                    memory_result.message)

            # 验证执行时间
            time_result = self.validate_execution_time(
                f"{name}_time", benchmark)
            validation_summary["total_tests"] += 1
        ...
        validation_summary["success_rate"] = (
            ...
            max(validation_summary["total_tests"], 1)
        )

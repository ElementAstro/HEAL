...
public class GameServer extends KcpServer {
  ...
  public GameServer(GameServerConfig serverConfig) {
    ...
    this.address = new InetSocketAddress(serverConfig.getBindAddress(),
  ...
  public GameServerConfig getServerConfig() { return this.serverConfig; }

  public void registerPlayer(Player player) {
    synchronized (this.players) { this.players.put(player.getUid(), player); }
  }
  ...
  public Int2ObjectMap<Player> getPlayer() { return this.players; }
  public Player getPlayerByUid(int uid, boolean allowOffline) {
    Player target = null;

    // Get player if online
    synchronized (this.players) { target = this.players.get(uid); }
  ...
  public boolean deletePlayer(String accountUid) {
    ...
    if (player == null) {
      player = LunarCore.getGameDatabase().getObjectByField(
          Player.class, "accountUid", accountUid);
      if (player == null)
        return false;
    }
  ...
  public void start() {
    ...
    LunarCore.getLogger().warn(
        "LUNARCORE IS A FREE SOFTWARE. IF YOU PAID FOR IT, YOU HAVE BEEN " +
        "SCAMMED!"); // DO NOT REMOVE. Anti-seller
  }
    ...
    synchronized (this.players) {
      for (Player player : this.players.values()) {
        try {
          ...
          LunarCore.getLogger().error(
              "[UID: " + player.getUid() + "] Player tick error: ", e);
        }
      }
    }
  }

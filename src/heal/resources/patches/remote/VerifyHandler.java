...
public final class VerifyHandler implements Handler {
  ...
  public void handle(@NotNull Context ctx) throws Exception {
    ...
    if (set_password == null || set_password.isEmpty()) {
      ctx.json(
          new JsonResponse(404, "The player remote password was not entered"));
      return;
    }
    ...
    LunarCore.getLogger().info(ip_address + " is setting remote password for " +
                               tmp_uid);
      ...
      if (sender != null) {
        ...
        if (code != null) {
          if (code.equals(tmp_code)) {
            sender.sendMessage(ip_address +
                               " is setting your remote password!");
            PasswordHandler.saveOrUpdatePassword(tmp_uid, set_password);
            sender.sendMessage("Your remote password is set as : " +
                               set_password);

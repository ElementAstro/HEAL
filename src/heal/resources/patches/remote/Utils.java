...
public class Utils {
  ...
  /**
   * Parses the string argument as a signed decimal integer. Returns a 0 if the
   * string argument is not an integer.
   */
  ...
  /**
   * Parses the string argument as a signed decimal long. Returns a 0 if the
   * string argument is not a long.
   */
  ...
  public static boolean isPortOpen(String host, int port) {
    try (var serverSocket = new ServerSocket()) {
      ...
      serverSocket.bind(
          new InetSocketAddress(InetAddress.getByName(host), port), 1);
      return true;
    } catch (Exception ex) {
      return false;
    }

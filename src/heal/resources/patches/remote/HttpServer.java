...
public class HttpServer {
  ...
  public Javalin getApp() { return this.app; }
  ...
  private SslContextFactory.Server getSSLContextFactory() {
    ...
    sslContextFactory.setKeyStorePath(
        LunarCore.getConfig().getKeystore().getPath());
    sslContextFactory.setKeyStorePassword(
        LunarCore.getConfig().getKeystore().getPassword());
    sslContextFactory.setSniRequired(false);
    sslContextFactory.setRenegotiationAllowed(false);
    return sslContextFactory;
  }

  public void forceRegionListRefresh() { this.nextRegionUpdate = 0; }
  ...
  public String getRegionList() {
    synchronized (this.regions) {
      ...
      if (System.currentTimeMillis() > this.nextRegionUpdate ||
        ...
        LunarCore.getAccountDatabase()
            .getObjects(RegionInfo.class)
            .forEach(region -> { this.regions.put(region.getId(), region); });

        // Serialize to proto
        DispatchRegionData regionData = DispatchRegionData.newInstance();
        regions.values()
            .stream()
            .map(RegionInfo::toProto)
            .forEach(regionData::addRegionList);

        // Set region list cache
        this.regionList = Utils.base64Encode(regionData.toByteArray());
        this.nextRegionUpdate =
            System.currentTimeMillis() + getServerConfig().regionListRefresh;
      }
    }
  ...
  public void start() {
    ...
    if (getServerConfig().isUseSSL()) {
      ServerConnector sslConnector =
          new ServerConnector(getApp().jettyServer().server(),
                              getSSLContextFactory(), getHttpFactory());
      ...
      getApp().start(getServerConfig().getBindAddress(),
                     getServerConfig().getBindPort());
    }

    // Done
    LunarCore.getLogger().info(
        "Http Server running as: " +
        this.modes.stream().collect(Collectors.joining(", ")));
    LunarCore.getLogger().info("Http Server started on " +
                               getServerConfig().getBindPort());
  }
  ...
  private void addDispatchRoutes() {
    ...
    getApp().post(
        ...
        new HttpJsonResponse(
            "{\"retcode\":0,\"message\":\"OK\",\"data\":{\"id\":\"none\"," +
            "\"action\":\"ACTION_NONE\",\"geetest\":null}}"));

    // === AUTHENTICATION === hkrpg-sdk-os-static.hoyoverse.com

    // Username & Password login (from client). Returns a session key to the
    // client.
    getApp().post("/hkrpg_global/mdk/shield/api/login",
                  new UsernameLoginHandler());
    // Cached session key verify (from registry). Returns a session key to the
    // client.
    getApp().post("/hkrpg_global/mdk/shield/api/verify",
                  new TokenLoginHandler());

    // Exchange session key for login token (combo token)
    getApp().post("/hkrpg_global/combo/granter/login/v2/login",
                  new ComboTokenGranterHandler());

    // Config
    getApp().get(
        "/hkrpg_global/combo/granter/api/getConfig",
        new HttpJsonResponse(
            "{\"retcode\":0,\"message\":\"OK\",\"data\":{\"protocol\":true," +
            "\"qr_enabled\":false,\"log_level\":\"INFO\",\"announce_url\":" +
            "\"\",\"push_alias_type\":0,\"disable_ysdk_guard\":true,\"enable_" +
            "announce_pic_popup\":false,\"app_name\":\"崩�??RPG\",\"qr_" +
            "enabled_apps\":{\"bbs\":false,\"cloud\":false},\"qr_app_icons\":" +
            "{\"app\":\"\",\"bbs\":\"\",\"cloud\":\"\"},\"qr_cloud_display_" +
            "name\":\"\",\"enable_user_center\":true,\"functional_switch_" +
            "configs\":{}}}"));
    getApp().get(
        "/hkrpg_global/mdk/shield/api/loadConfig",
        new HttpJsonResponse(
            "{\"retcode\":0,\"message\":\"OK\",\"data\":{\"id\":24,\"game_" +
            "key\":\"hkrpg_global\",\"client\":\"PC\",\"identity\":\"I_" +
            "IDENTITY\",\"guest\":false,\"ignore_versions\":\"\",\"scene\":" +
            "\"S_NORMAL\",\"name\":\"崩�??RPG\",\"disable_regist\":false," +
            "\"enable_email_captcha\":false,\"thirdparty\":[\"fb\",\"tw\"," +
            "\"gl\",\"ap\"],\"disable_mmt\":false,\"server_guest\":false," +
            "\"thirdparty_ignore\":{},\"enable_ps_bind_account\":false," +
            "\"thirdparty_login_configs\":{\"tw\":{\"token_type\":\"TK_GAME_" +
            "TOKEN\",\"game_token_expires_in\":2592000},\"ap\":{\"token_" +
            "type\":\"TK_GAME_TOKEN\",\"game_token_expires_in\":604800}," +
            "\"fb\":{\"token_type\":\"TK_GAME_TOKEN\",\"game_token_expires_" +
            "in\":2592000},\"gl\":{\"token_type\":\"TK_GAME_TOKEN\",\"game_" +
            "token_expires_in\":604800}},\"initialize_firebase\":false,\"bbs_" +
            "auth_login\":false,\"bbs_auth_login_ignore\":[],\"fetch_" +
            "instance_id\":false,\"enable_flash_login\":false}}"));
    ...
    getApp().get(
        ...
        new HttpJsonResponse(
            "{\"retcode\":0,\"message\":\"OK\",\"data\":{\"vals\":{\"kibana_" +
            "pc_config\":\"{ \\\"enable\\\": 0, \\\"level\\\": " +
            "\\\"Info\\\",\\\"modules\\\": [\\\"download\\\"] " +
            "}\\n\",\"network_report_config\":\"{ \\\"enable\\\": 0, " +
            "\\\"status_codes\\\": [206], \\\"url_paths\\\": " +
            "[\\\"dataUpload\\\", \\\"red_dot\\\"] " +
            "}\\n\",\"list_price_tierv2_enable\":\"false\\n\",\"pay_payco_" +
            "centered_host\":\"bill.payco.com\",\"telemetry_config\":\"{\\n " +
            "\\\"dataupload_enable\\\": " +
            "0,\\n}\",\"enable_web_dpi\":\"true\"}}}"));
    getApp().get(
        "/combo/box/api/config/sw/precache",
        new HttpJsonResponse("{\"retcode\":0,\"message\":\"OK\",\"data\":{" +
                             "\"vals\":{\"url\":\"\",\"enable\":\"false\"}}}"));

    // sg-public-data-api.hoyoverse.com
    getApp().get("/device-fp/api/getFp", new FingerprintHandler());
    getApp().get("/device-fp/api/getExtList",
                 new HttpJsonResponse(
                     "{\"retcode\":0,\"message\":\"OK\",\"data\":{\"code\":" +
                     "200,\"msg\":\"ok\",\"ext_list\":[],\"pkg_list\":[]," +
                     "\"pkg_str\":\"/vK5WTh5SS3SAj8Zm0qPWg==\"}}"));

    // abtest-api-data-sg.hoyoverse.com
    getApp().post("/data_abtest_api/config/experiment/list",
                  new HttpJsonResponse(
                      "{\"retcode\":0,\"success\":true,\"message\":\"\"," +
                      "\"data\":[{\"code\":1000,\"type\":2,\"config_id\":" +
                      "\"14\",\"period_id\":\"6125_197\",\"version\":\"1\"," +
                      "\"configs\":{\"cardType\":\"direct\"}}]}"));

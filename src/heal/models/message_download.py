...
class MessageLauncher(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "选择启动器版本:",
            "当前内容包含以下项目:",
            [f"Firefly-Launcher:{cfg.APP_VERSION}"],
            parent,
        )
...
class MessagePython(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:", "当前内容包含以下项目:", ["Python: Python环境"], parent
        )
...
class MessageCompiler(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:", "当前内容包含以下项目:", ["Compiler: 编译环境"], parent
        )
...
class MessageNINA(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:", "当前内容包含以下项目:", ["NINA: NINA项目本体"], parent
        )
...
class MessagePHD2(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:", "当前内容包含以下项目:", ["PHD2: PHD2项目本体"], parent
        )
...
class MessageGit(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:", "当前内容包含以下项目:", ["Git: Git环境"], parent
        )
...
class MessageJava(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:", "当前内容包含以下项目:", ["Java: Java环境"], parent
        )
...
class MessageLunarCoreRes(MessageDialog):
    def __init__(self, parent: Optional[QWidget] = None) -> None:
        super().__init__(
            "确认当前下载项目:",
            "当前内容包含以下项目:",
            ["LunarCoreRes: StarRailData和Configs"],
            parent,
        )
...
class MessageDownload(MessageBoxBase):
    ...
    def download_finished(self, success: bool, file_path: str) -> None:
...
class CommandRunner(QThread):
    ...
    def run(self) -> None:
        ...
        if self.build_jar == "lunarcore":
            ...
            subprocess.run(
                "cd server/LunarCore && gradlew jar",
                shell=True,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
            )

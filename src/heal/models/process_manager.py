...
class ProcessMonitor(QThread):
    ...
    def __init__(self, process_manager: Any) -> None:
        super().__init__()
...
class ProcessManager(QObject):
    ...
    def start_process(self, name: str, **kwargs: Any) -> bool:
        """启动进程"""
        try:
    ...
    def restart_process(self, name: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    f"Process {name} restarted (count: {self.processes[name].restart_count})"
                )
                self.process_restarted.emit(
                    name
                )  # Emit restart signal on successful restart
            else:
    ...
    def _handle_process_crash(self, name: str) -> None:
        """处理进程崩溃"""
        try:
            process_info = self.processes[name]
    ...
    def _start_output_monitor(self, name: str, proc: Any) -> None:
        """启动输出监控线程"""
    ...
    def _write_to_log(self, name: str, message: str) -> None:
        """写入日志文件"""
        try:
    ...
    def _on_status_changed(self, name: str, status: ProcessStatus) -> None:
        """状态变化处理"""
        if name in self.processes:
    ...
    def save_state(self, file_path: Optional[str] = None) -> None:
        ...
        try:
            ...
            state = {
                "processes": {
                    name: info.to_dict() for name, info in self.processes.items()
                }
            }

...
class EnvironmentConfigManager(QObject):
    ...
    def load_download_config(self) -> List[SettingCard]:
        ...
        result = JsonUtils.load_json_file(
            self.config_file, create_if_missing=False, default_content=[]
        )
        ...
        for item in self.config_data:
            try:
                ...
                if card:
                    ...
                    self.logger.info(
                        f"Loaded card from config: {item.get('title', 'Unknown')}"
                    )
    ...
    def remove_config_item(self, title: str) -> bool:
        ...
        try:
            self.config_data = [
                item for item in self.config_data if item.get("title") != title
            ]
            return self.save_config(self.config_data)
        except Exception as e:

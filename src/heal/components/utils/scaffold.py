...
def query_user() -> tuple[str, str, str, str, str, str, str, str, str, str]:
    print("Please provide the following information for your new module:")
    module_name = input("Module Name (e.g., atom.utils): ").strip()
    component_name = input(
        "Component Name (the name of the shared_ptr, should be considered seriously, e.g. SystemComponent): "
    ).strip()
...
def confirm_details(details: Any) -> tuple[bool, bool]:
...
def create_cmakelists(
    module_name: str, cpp_standard: str, additional_sources: str, additional_headers: str
) -> str:
...
def create_source(module_name: str) -> str:
...
def create_package_json(module_name: str, version: str, description: str, author: str, license: str, repo_url: str) -> str:
    package = {
        "name": module_name,
        "version": version,
        "type": "shared",
        ...
        "homepage": repo_url,
        "keywords": ["lithium", *module_name.split(".")],
        "scripts": {
            "build": "cmake --build . --config Release -- -j 4",
            "lint": "clang-format -i src/*.cpp include/*.h",
        },
        "modules": [{"name": module_name.split(".")[-1], "entry": "getInstance"}],
    }
    return json.dumps(package, indent=4)


def create_readme(module_name: str, description: str, author: str, version: str) -> str:
...
def create_component_main(author: str, module_name: str, component_name: str) -> str:
    content = f"""/*
 * _main.cpp
 *
...
def create_component_hpp(author: str, description: str, module_name: str, component_name: str) -> str:
    content = f"""
/*
 * _component.hpp
 *
...
def create_component_cpp(author: str, description: str, module_name: str, component_name: str) -> str:
    content = f"""/*
 * _component.cpp
 *
 * Copyright (C) 2023-2024 {author}
...
def main() -> None:
    ...
    with open(os.path.join(module_dir, "CMakeLists.txt"), "w") as f:
        f.write(
            create_cmakelists(
                module_name, cpp_standard, additional_sources, additional_headers
            )
        )
    ...
    with open(os.path.join(module_dir, "package.json"), "w") as f:
        f.write(
            create_package_json(
                module_name, version, description, author, license, repo_url
            )
        )

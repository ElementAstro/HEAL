...
from PySide6.QtGui import QContextMenuEvent, QFont, QPalette
...
class ServerStatusCard(QFrame):
    ...
    def __init__(
        self, server_name: str, server_config: Dict[str, Any], parent: QWidget | None = None
    ) -> None:
        super().__init__(parent)
        self.server_name = server_name
        self.server_config = server_config
        self.current_status = "stopped"  # stopped, running, error, starting, stopping
        self.process_id: Any = None
        self.port = server_config.get("port", "N/A")

        self.logger = get_logger(
            "server_status_card", module=f"ServerStatusCard-{server_name}"
        )
    ...
    def _connect_signals(self) -> None:
        ...
        self.start_btn.clicked.connect(
            lambda: self.start_requested.emit(self.server_name)
        )
        self.stop_btn.clicked.connect(
            lambda: self.stop_requested.emit(self.server_name)
        )
        self.restart_btn.clicked.connect(
            lambda: self.restart_requested.emit(self.server_name)
        )
        self.settings_btn.clicked.connect(
            lambda: self.settings_requested.emit(self.server_name)
        )
    ...
    def update_status(self, status: str, process_id: Optional[int] = None) -> None:
    ...
    def _view_server_logs(self) -> None:
        """View server logs."""

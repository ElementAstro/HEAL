...
class SettingsSearchEngine(QObject):
    ...
    def search(
        ...
        try:
            ...
            self.logger.debug(
                f"Search completed in {search_time:.3f}s, {len(results)} results"
            )
    ...
    def search_realtime(
        self, query: str, filters: List[SearchFilter] | None = None, delay_ms: int = 300
    ) -> None:
    ...
    def _apply_single_filter(self, filter_obj: SearchFilter) -> Set[str]:
        ...
        if filter_obj.filter_type == FilterType.CATEGORY:
            ...
            if callable(filter_obj.value):
                return {
                    key for key, item in self.items.items() if filter_obj.value(item)
                }

        return set()

    def _search_items(
        self, query: str, candidates: Set[str], search_type: SearchType
    ) -> List[SearchResult]:
    ...
    def _highlight_matches(self, item: SettingItem, query: str) -> str:
        ...
        highlighted = re.sub(
            f"({re.escape(query)})", r"**\1**", text, flags=re.IGNORECASE
        )
        return highlighted
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "advanced_settings": sum(
                1 for item in self.items.values() if item.is_advanced
            ),
        }

...
from PySide6.QtGui import QAction, QFont, QIcon, QPainter, QPalette, QPixmap
...
class SearchFilterWidget(CardWidget):
    ...
    def _on_filter_changed(self) -> None:
        ...
        if category != "All Categories":
            self.filters.append(
                SearchFilter(filter_type=FilterType.CATEGORY, value=category)
            )
        ...
        if setting_type != "All Types":
            self.filters.append(
                SearchFilter(filter_type=FilterType.TYPE, value=setting_type)
            )
        ...
        if self.favorites_checkbox.isChecked():
            self.filters.append(
                SearchFilter(filter_type=FilterType.FAVORITES, value=True)
            )
...
class SearchResultsWidget(QScrollArea):
    ...
    def update_results(self, results: List[SearchResult], query: str = "") -> None:
...
class SettingsSearchWidget(QWidget):
    ...
    def __init__(self, search_engine: SettingsSearchEngine, parent=None) -> None:
        ...
        self.logger = get_logger(
            "settings_search_widget", module="SettingsSearchWidget"
        )
    ...
    def setup_search_type_menu(self) -> None:
        ...
        keyword_action.triggered.connect(
            lambda: self.set_search_type(SearchType.KEYWORD)
        )
    ...
    def on_search_completed(self, results: List[SearchResult]) -> None:
    ...
    def on_suggestion_selected(self, suggestion: str) -> None:
        """Handle suggestion selection"""
        self.search_input.setText(suggestion)
        self.perform_search()

    def on_filters_changed(self, filters: List[SearchFilter]) -> None:
        """Handle filter changes"""
        query = self.search_input.text().strip()
        if query or filters:
            self.search_engine.search(query, filters, self.current_search_type)

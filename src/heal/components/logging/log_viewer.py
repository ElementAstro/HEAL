...
class LogFileWatcher(QThread):
    ...
    def __init__(self, log_files: Dict[str, Path]) -> None:
        super().__init__()
        self.log_files = log_files
        self.file_positions: dict[str, Any] = {}
        self.running = True
...
class LogViewer(CardWidget):
    ...
    def _load_initial_content(self, name: str, path: Path, text_widget: QTextEdit) -> None:
        """加载初始日志内容"""
        try:
            if path.exists():
                with open(path, "r", encoding="utf-8") as f:
    ...
    def _on_new_lines(self, file_name: str, new_lines: List[str]) -> None:
        """处理新的日志行"""
    ...
    def _limit_text_lines(self, text_widget: QTextEdit) -> None:
        """限制文本行数"""
    ...
    def apply_filter(self, filters: Dict) -> None:
        """应用过滤条件"""
        self.current_filters = filters
        # TODO: 重新加载并过滤日志内容

    def export_logs(self, export_config: Dict) -> None:
        """导出日志"""
        # TODO: 实现日志导出功能
        pass

...
class ModDetailPage(QFrame):
    def __init__(self, mod_info: ModInfo, parent_manager: "ModManager") -> None:
        super().__init__()
...
class AddModDialog(QDialog):
    def __init__(self, parent_manager: "ModManager") -> None:
...
class ModManager(ScrollArea):
    ...
    def _setup_system_connections(self) -> None:
        ...
        self.bulk_operations.operation_progress.connect(
            self._on_bulk_operation_progress
        )

        # Register bulk operation handlers
        self.bulk_operations.register_operation_handler(
            BulkOperationType.ENABLE, self._handle_bulk_enable
        )
        self.bulk_operations.register_operation_handler(
            BulkOperationType.DISABLE, self._handle_bulk_disable
        )
        self.bulk_operations.register_operation_handler(
            BulkOperationType.DELETE, self._handle_bulk_delete
        )
        self.bulk_operations.register_operation_handler(
            BulkOperationType.VALIDATE, self._handle_bulk_validate
        )
                ...
                if os.path.isfile(package_file):
                    try:
                        with open(package_file, "r", encoding="utf-8") as f:
                            ...
                            thumbnail = (
                                os.path.join(mod_path, "thumbnail.png")
                                if os.path.isfile(
                                    os.path.join(mod_path, "thumbnail.png")
                                )
                                else None
                        ...
                        QMessageBox.critical(
                            self, "错误", f"加载模组 '{mod_dir}' 时发生错误: {e}"
                        )
    ...
    def update_mod_list(self) -> None:
        ...
        for info in self.filtered_mods.values():
            ...
            checkbox.toggled.connect(
                partial(self._on_module_selection_changed, info.name)
            )
            ...
            workflow_button.clicked.connect(
                partial(self.start_module_workflow, info.name)
            )
    ...
    def filter_mods(self) -> None:
        ...
        if search_text:
            self.filtered_mods = {
                name: info
                for name, info in self.mods.items()
    ...
    def show_mod_details(self, item: QListWidgetItem) -> None:
    ...
    def delete_mod(self, mod_name: str) -> None:
        if mod_name in self.mods:
            ...
            try:
                ...
                QMessageBox.critical(
                    self, "错误", f"删除模组 '{mod_name}' 时发生错误: {e}"
                )
            self.load_mods()
    ...
    def copy_mod_details(self, mod_info: ModInfo) -> None:
        details = {
            "Name": mod_info.name,
            "Author": mod_info.author,
            "Version": mod_info.version,
    ...
    def _on_module_selection_changed(self, module_name: str, checked: bool) -> None:
    ...
    def _toggle_module_selection(self, module_name: str) -> None:
    ...
    def start_module_workflow(self, module_name: str) -> None:
    ...
    def bulk_enable_selected(self) -> None:
    ...
    def bulk_disable_selected(self) -> None:
    ...
    def bulk_validate_selected(self) -> None:
    ...
    def bulk_delete_selected(self) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            self._save_state_for_undo(
                "bulk_delete", list(self.selected_modules), backup_data
            )
            operation_id = self.bulk_operations.delete_selected_modules()
            self._show_bulk_operation_progress(operation_id)

    def _show_bulk_operation_progress(self, operation_id: str) -> None:
    ...
    def _on_workflow_step_failed(self, workflow_id: str, step: str, error: str) -> None:
    ...
    def _on_bulk_operation_progress(
        ...
        if (
            ...
            and operation_id == self.current_bulk_operation
        ):

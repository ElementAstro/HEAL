...
class ValidationReportWidget(QScrollArea):
    ...
    def update_report(self, report: ModuleValidationReport) -> None:
        """更新验证报告"""
    ...
    def _create_info_card(self) -> CardWidget:
        ...
        info_layout.addWidget(BodyLabel(report.validation_level.value), 1, 1)
        ...
        info_layout.addWidget(BodyLabel(str(len(report.issues))), 3, 1)
    ...
    def _create_issues_card(self) -> CardWidget:
        ...
        # 标题
        title_label = SubtitleLabel(f"验证问题 ({len(report.issues)})")
        layout.addWidget(title_label)
...
class ModuleValidationUI(QWidget):
    ...
    def _create_control_panel(self) -> QGroupBox:
        ...
        self.level_combo.addItems(["基础验证", "标准验证", "严格验证", "安全验证"])
    ...
    def _on_batch_validate_clicked(self) -> None:
        """批量验证按钮点击"""
        from PySide6.QtWidgets import QFileDialog
    ...
    def _start_validation(self, module_path: str) -> None:
        """开始验证"""
    ...
    def _start_batch_validation(self, file_paths: list[str]) -> None:
        """开始批量验证"""
    ...
    def _update_validation_result(self, report: ModuleValidationReport) -> None:
        """更新验证结果"""
        result_text = report.overall_result.value.upper()
    ...
    def _show_batch_results(self, reports: list[ModuleValidationReport]) -> None:
        """显示批量验证结果"""
...
成功率: {summary['success_rate']:.1f}%"""
    ...
    def _export_report(self, file_path: str) -> None:
        """导出报告"""
        try:
            if not self.current_validation_report:
                return

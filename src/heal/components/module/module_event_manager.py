...
class ModuleEventManager(QObject):
    ...
    def emit_module_loaded(self, module_name: str) -> None:
        """发出模块加载信号"""
        self.module_loaded.emit(module_name)

    def emit_module_unloaded(self, module_name: str) -> None:
        """发出模块卸载信号"""
        self.module_unloaded.emit(module_name)

    def emit_module_error(self, module_name: str, error_message: str) -> None:
        """发出模块错误信号"""
        self.module_error.emit(module_name, error_message)

    def emit_performance_updated(self, module_name: str, metrics: dict) -> None:
        """发出性能更新信号"""
        self.performance_updated.emit(module_name, metrics)

    def emit_validation_completed(self, module_name: str, success: bool, issues: list) -> None:
        """发出验证完成信号"""

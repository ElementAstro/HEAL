...
class ModDownloadThread(QThread):
    ...
    def __init__(self, url: str, save_path: str, proxy: Optional[str] = None) -> None:
        super().__init__()
        self.url = url
...
class SettingsWindow(QDialog):
    ...
    def __init__(self, settings: Dict[str, Any], parent: Optional[Any] = None) -> None:
...
class ModDownload(ScrollArea):
    ...
    def display_mod_details(self, item: QListWidgetItem) -> None:
        """显示模组详情"""
        ...
        download_button.clicked.connect(
            lambda: self.manage_mod(selected_mod, download_button)
        )
        ...
        copy_name_button.clicked.connect(
            lambda: self.copy_to_clipboard(selected_mod.name)
        )

        # 复制模组描述按钮
        copy_desc_button = ToolButton(FluentIcon.COPY)
        copy_desc_button.setToolTip("复制模组描述")
        copy_desc_button.clicked.connect(
            lambda: self.copy_to_clipboard(selected_mod.description)
        )
    ...
    def manage_mod(self, mod: Mod, button: PushButton) -> None:
        """下载或卸载模组"""
        if mod.installed:
    ...
    def uninstall_mod(self, mod: Mod, button: PushButton) -> None:
        """卸载模组"""
        reply = QMessageBox.question(
            self,
            "确认卸载",
            f"确定要卸载模组 {mod.name} 吗？",
    ...
    @Slot(bool, str)  # type: ignore[misc]
    def finish_download(self, mod: Mod, button: PushButton, success: bool, info: str) -> None:
        """下载完成"""
        if success:
            mod.installed = True
            button.setText("卸载")
    ...
    def filter_mod_list(self) -> None:
        ...
        for mod in self.mods:
            if (
                keyword not in mod.name.lower()
                and keyword not in mod.description.lower()
            ):
                continue

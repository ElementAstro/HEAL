        ...
        try:
            ...
            metrics = {
                ...
                "total_operations": sum(
                    m.get("operations", 0) for m in self.modules_data.values()
                ),
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "disk_usage": {
                    disk.device: f"{psutil.disk_usage(disk.mountpoint).percent:.1f}%"
            ...
            details.update(
                {
                    ...
                    "last_activity": datetime.fromtimestamp(
                        details.get("last_activity", time.time())
        ...
        try:
            ...
            if len(self.values) > 1:
                ...
                if len(self.times) > 5:
                    ...
                    tick_labels = [
                        self.times[i].strftime("%H:%M:%S") for i in tick_positions
                    ]
...
class MetricsDisplayWidget:
    ...
    def __init__(self, parent) -> None:
        ...
        # Metrics labels
        self.metrics_vars: dict[str, Any] = {}
...
class AlertsWidget:
    ...
    def __init__(self, parent) -> None:
        ...
        # Clear button
        ttk.Button(self.frame, text="Clear Alerts", command=self.clear_alerts).pack(
            pady=(5, 0)
        )
...
class PerformanceDashboardUI:
    ...
    def setup_system_tab(self) -> None:
    ...
    def setup_settings_tab(self) -> None:
        ...
        ttk.Button(
            controls_frame, text="Apply Settings", command=self.apply_settings
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(controls_frame, text="Reset Charts", command=self.reset_charts).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(controls_frame, text="Export Data", command=self.export_data).pack(
            side=tk.LEFT, padx=(0, 5)
        )
    ...
    def update_modules_tree(self, modules_data: Dict[str, Any]) -> None:

...
class ScaffoldApp(QWidget):
    ...
    def create_intro_page(self) -> None:
        intro_page = QWidget()
    ...
    def set_mode_and_next(self) -> None:
    ...
    def create_form_pages(self) -> None:
        self.pages: list[Any] = []
        fields = [
            ...
            (
                "额外的源文件 (用逗号分隔, 例如 utils.cpp,math.cpp):",
                "additional_sources",
            ),
            (
                "额外的头文件 (用逗号分隔, 例如 utils.hpp,math.hpp):",
                "additional_headers",
            ),
        ]
    ...
    def collect_all_inputs(self) -> None:
        methods = [
            "module_name",
            "component_name",
            "version",
    ...
    def confirm_details(self) -> None:
        ...
        if not confirmed:
            CustomMessageBox.warning(self, "警告", "请检查您的输入并重试。")
        ...
        try:
            ...
            (module_dir / "CMakeLists.txt").write_text(
                create_cmakelists(
                    ...
                    ",".join(self.module_details.additional_sources),
                    ",".join(self.module_details.additional_headers),
                ),
                encoding="utf-8",
            )

            (include_dir / f"{base_name}.hpp").write_text(
                create_header(self.module_details.module_name), encoding="utf-8"
            )

            (src_dir / f"{base_name}.cpp").write_text(
                create_source(self.module_details.module_name), encoding="utf-8"
            )

            for src in filter(None, self.module_details.additional_sources):
                (src_dir / src.strip()).write_text(
                    "// {}\n".format(src.strip()), encoding="utf-8"
                )

            for hdr in filter(None, self.module_details.additional_headers):
            ...
            (module_dir / ".gitignore").write_text(create_gitignore(), encoding="utf-8")

...
from qfluentwidgets import (
    ScrollArea, PrimaryPushSettingCard, FluentIcon,
...
from app.components.environment.enhanced_cards import (
    EnvironmentStatusCard, SmartToolCard, ToolCategorySection, QuickActionPanel
)
from app.components.environment.tool_status_manager import ToolStatusManager, ToolStatus
from app.components.environment.platform_detector import get_current_platform_info
...
class Environment(ScrollArea):
    ...
    def __init__(self, text: str, parent: Optional[QWidget] = None):
        ...
        self.runtime_section = None
        self.database_section = None
        self.version_control_section = None
    ...
    def create_tool_categories(self):
        ...
        self.runtime_section = ToolCategorySection(
            "运行时环境",
            "Java, Node.js, Python 等开发运行时"
        )
        self.vBoxLayout.addWidget(self.runtime_section)

        # 数据库
        self.database_section = ToolCategorySection(
            "数据库",
            "MongoDB 等数据库系统"
        )
    ...
    def handle_quick_action(self, action_type: str, target: str):
        """处理快速操作"""
    ...
    def handle_download_request(self, tool_name: str, option_key: str):
        """处理下载请求"""
        # 使用现有的下载处理逻辑
        self.signal_manager.handle_download_started(option_key)
        logger.info(f"下载请求: {tool_name} - {option_key}")

    def on_tool_status_updated(self, tool_name: str, tool_info):
        """工具状态更新回调"""
        # 更新对应的状态卡片
        self.update_status_card(tool_name, tool_info)

    def on_all_status_updated(self, all_status: Dict):
        """所有工具状态更新回调"""
        for tool_name, tool_info in all_status.items():
            self.update_status_card(tool_name, tool_info)

    def update_status_card(self, tool_name: str, tool_info):
        """更新状态卡片"""
        # 查找或创建状态卡片
    ...
    def find_or_create_status_card(self, tool_name: str, tool_info):
        """查找或创建状态卡片"""
        # 在状态区域查找现有卡片
        if hasattr(self.status_section, 'findChild'):
            existing_card = self.status_section.findChild(EnvironmentStatusCard, f"status_{tool_name}")
            if existing_card:
    ...
    def handle_tool_launch(self, tool_name: str):
        """处理工具启动"""
    ...
    def handle_tool_install(self, tool_name: str):
        """处理工具安装"""
        logger.info(f"安装工具请求: {tool_name}")

    def handle_tool_update(self, tool_name: str):
        """处理工具更新"""
        logger.info(f"更新工具请求: {tool_name}")

    def handle_tool_configure(self, tool_name: str):
        """处理工具配置"""
    ...
    def load_download_config(self) -> None:
        ...
        for item in config_data:
            if item.get('type') == 'link':
                ...
                if self.download_section and hasattr(self.download_section, 'add_tool_card'):

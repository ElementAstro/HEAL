...
from app.components.main import (
    WindowManager, MainNavigationManager, AuthenticationManager,
    ThemeManager, AudioManager, FontManager, UpdateManager
)
...
class Main(MSFluentWindow):
    ...
    def connect_signals(self):
        """连接所有信号"""
        # 导航管理器信号
        self.navigation_manager.theme_change_requested.connect(self.theme_manager.toggle_theme)
        self.navigation_manager.reload_requested.connect(lambda: self.reload_signal.emit())
        self.navigation_manager.shutdown_requested.connect(lambda: self.shutdown_signal.emit())
        ...
        # 更新管理器信号
        self.update_manager.update_checked.connect(self.update_manager.handle_update_result)
    ...
    def handle_login_failed(self, attempt_count: int):
        """处理登录失败"""
        if cfg.useAudio.value:

...
from qfluentwidgets import (TogglePushButton, FluentIcon, setCustomStyleSheet)
from app.model.config import cfg
from app.model.process_manager import ProcessManager
from app.common.exception_handler import ExceptionHandler
from app.components.home.server_button import ServerButton
...
class ServerManager:
    ...
    def update_button_state(self, process_name: str, state: str):
        """更新按钮状态"""
        for button_widget in self.button_group.buttons():
            if button_widget.objectName() == process_name:
                if state == "started":
                    button_widget.setChecked(True)
                    button_widget.setStyleSheet('background-color: #4CAF50; color: white;')
                elif state == "stopped":
                    button_widget.setChecked(False)
                    button_widget.setStyleSheet('')
                elif state == "crashed":
                    button_widget.setStyleSheet('background-color: #F44336; color: white;')
                break

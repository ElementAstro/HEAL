...
class StatusOverviewWidget(QFrame):
    ...
    def _update_status_summary(self):
        ...
        running_servers = sum(1 for status in self.server_statuses.values() 
                             if status == "running")
    ...
    def _update_server_indicators(self):
    ...
    def get_system_summary(self) -> Dict[str, int]:
        ...
        running = sum(1 for status in self.server_statuses.values() if status == "running")
        stopped = sum(1 for status in self.server_statuses.values() if status == "stopped")

...
from PySide6.QtWidgets import (QDialog, QVBoxLayout, QFormLayout, QLabel, QWidget)
from PySide6.QtGui import QAction
from qfluentwidgets import (TextEdit, LineEdit, PushButton, InfoBar, InfoBarIcon, 
                            InfoBarPosition, PrimaryPushButton)
...
class DialogManager:
    ...
    def show_config_dialog(self, server_name: str):
        """显示服务器配置对话框"""
    ...
    def save_config(self, server_name: str, config_path: str, dialog_to_close: QDialog):
        """保存服务器配置"""
        try:
    ...
    def show_error_info_bar(self, name: str, parent_stackedwidget=None):
        """显示错误信息栏"""
        ...
        if (name in server_page and
                parent_stackedwidget is not None and
                hasattr(parent_stackedwidget, 'setCurrentIndex')):
            
            page_index = server_page[name]
            server_error_button = PrimaryPushButton('前往下载')
            server_error_button.clicked.connect(
                lambda: parent_stackedwidget.setCurrentIndex(page_index)
                if parent_stackedwidget else None
            )

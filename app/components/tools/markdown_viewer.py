...
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QVBoxLayout, QHBoxLayout, QTextBrowser,
    QWidget, QFileDialog
)
import markdown
...
class MarkdownEditor(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.export_pdf_button = PushButton(
            text="Export to PDF",
            icon=FluentIcon.DOCUMENT  # Assume FILE_PDF exists
        )
    ...
    def on_file_loaded(self, markdown_text):
    ...
    def save_markdown_file(self):
        ...
        if file_path:
            try:
                ...
                MessageBox.information(self, "Success", "Markdown file saved successfully.")
            except Exception as e:
    ...
    def export_to_html(self):
        ...
        if file_path:
            try:
                html = markdown.markdown(self.editor.toPlainText(), extensions=['fenced_code', 'tables'])
                with open(file_path, "w", encoding="utf-8") as file:
                    file.write(html)
                logger.info(f"Exported HTML file: {file_path}")
                MessageBox.information(self, "Success", "Exported to HTML successfully.")
            except Exception as e:
    ...
    def apply_theme(self):
        ...
        self.setStyleSheet(f"background-color: {self.current_theme.background}; color: {self.current_theme.color};")
        self.apply_theme_to_editor()
        self.apply_theme_to_preview()
        self.update_preview()

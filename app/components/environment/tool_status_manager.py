...
from .platform_detector import get_current_platform_info, PlatformType
...
class ToolStatusManager(QObject):
    ...
    def _check_mongodb_status(self) -> ToolInfo:
        ...
        if success and stdout:
            lines = stdout.split('\n')
            version_line = next((line for line in lines if 'version' in line.lower()), "")
            if version_line:

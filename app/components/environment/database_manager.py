...
class EnvironmentDatabaseManager(QObject):
    ...
    def __init__(self, parent_widget: QWidget):
        ...
        self.logger = get_logger('environment_db_manager', module='EnvironmentDatabaseManager')
    ...
    def handle_mongo_db_open(self) -> None:
        ...
        if self.mongo_exe.exists():
            try:
                ...
                Info(self.parent_widget, "S", 1000, self.parent_widget.tr("数据库已开始运行!"))
                self.database_started.emit()
                self.logger.info("MongoDB 已启动。")
            except Exception as e:
                error_msg = str(e)
                Info(self.parent_widget, 'E', 3000, self.parent_widget.tr("启动数据库失败！"), error_msg)
                self.database_start_failed.emit(error_msg)
                self.logger.error(f"启动 MongoDB 失败: {e}")
        else:
            self._show_database_not_found_error()
    
    def _show_database_not_found_error(self):
        """显示数据库未找到错误"""
        file_error = InfoBar(
    ...
    def get_database_status(self) -> dict:
        ...
        return {
            ...
            'executable': self.mongo_exe.is_file() if self.mongo_exe.exists() else False
        }

...
from .environment_cards import PrimaryPushSettingCardDownload, HyperlinkCardEnvironment
...
class EnvironmentSignalManager(QObject):
    ...
    def __init__(self, parent_widget, download_interface_widget):
        super().__init__(parent_widget)
        self.parent_widget = parent_widget
        self.download_interface_widget = download_interface_widget
        self.logger = get_logger('environment_signal_manager', module='EnvironmentSignalManager')
        
        # 初始化下载命令处理器
        self.sub_download_cmd = SubDownloadCMD(parent_widget)
    
    def connect_card_signals(self, cards_container):
        """连接卡片信号"""
    ...
    def connect_database_signals(self, database_manager):
        """连接数据库管理器信号"""

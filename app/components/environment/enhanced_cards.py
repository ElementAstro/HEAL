...
from qfluentwidgets import (
    FluentIcon, PrimaryPushButton, PushButton, InfoBadge,
    BodyLabel, CaptionLabel, IconWidget, ToolTip, ComboBox,
    ProgressBar, IndeterminateProgressBar
)

from app.model.setting_card import SettingCard, SettingCardGroup
from app.common.logging_config import get_logger
from .tool_status_manager import ToolStatus, ToolInfo
...
class EnvironmentStatusCard(SettingCard):
    ...
    def __init__(self, tool_name: str, tool_info: ToolInfo, parent=None):
        # 根据状态选择图标
        ...
        self.logger = get_logger('environment_status_card', module='EnvironmentStatusCard')
    ...
    def update_status(self, tool_info: ToolInfo):
        """更新状态显示"""
        self.tool_info = tool_info
    ...
    def _update_status_badge(self, status: ToolStatus):
        """更新状态徽章"""
        status_configs = {
    ...
    def _update_buttons(self, status: ToolStatus):
        """更新按钮状态"""
...
class SmartToolCard(SettingCard):
    ...
    def __init__(self, title: str, content: str, options: List[Dict], 
                 tool_status: Optional[ToolStatus] = None, parent=None):
...
class ToolCategorySection(SettingCardGroup):
    ...
    def add_tool_card(self, card: SettingCard):
        """添加工具卡片"""
        self.addSettingCard(card)
        self.logger.debug(f"工具卡片已添加到分类 {self.title}: {card.titleLabel.text()}")

...
from PySide6.QtWidgets import (
    ...
    QComboBox, QLineEdit, QCheckBox, QDateTimeEdit,
    QGroupBox, QPushButton, QLabel
)
from PySide6.QtCore import Qt, Signal, QDateTime

from qfluentwidgets import (
    FluentIcon as FIF, ComboBox, LineEdit, CheckBox, 
    PushButton, BodyLabel, TitleLabel, CardWidget
)
...
class LogFilter(CardWidget):
    ...
    def _create_level_filter(self, parent_layout: QVBoxLayout):
        """创建日志级别过滤"""
    ...
    def _create_time_filter(self, parent_layout: QVBoxLayout):
        """创建时间范围过滤"""
    ...
    def _create_module_filter(self, parent_layout: QVBoxLayout):
        """创建模块过滤"""
        ...
        # 常用模块快速选择
        common_modules = [
            "main_interface", "download_interface", "module_interface",
            "proxy_interface", "logging_config", "process_monitor"
        ]
    ...
    def _create_keyword_filter(self, parent_layout: QVBoxLayout):
        """创建关键词搜索"""
    ...
    def _on_module_check_toggled(self, checked: bool):
        """模块复选框改变"""
    ...
    def _select_no_levels(self):
        """取消选择所有日志级别"""
        for check in self.level_checks.values():
            check.setChecked(False)
        self._update_level_filter()
    
    def _set_quick_time(self, **kwargs):
        """设置快速时间范围"""
    ...
    def _update_time_filter(self):
        """更新时间过滤"""
        if self.time_filter_enabled.isChecked():
            self.current_filters['start_time'] = self.start_time_edit.dateTime().toPython()
    ...
    def _update_module_filter_from_checks(self):
        """从复选框更新模块过滤"""

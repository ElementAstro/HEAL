...
class LogIntegrationManager(QObject):
    ...
    def _register_log_sources(self):
        """注册日志源"""
        # 服务器日志
    ...
    def show_server_log_in_panel(self, server_name: str):
        """在统一面板中显示服务器日志"""
        try:
    ...
    def show_process_log_in_panel(self, process_name: str):
        """在统一面板中显示进程日志"""
        try:
            # 切换到进程管理日志标签
    ...
    def show_performance_log_in_panel(self, operation_name: Optional[str] = None):
        """在统一面板中显示性能日志"""
        try:
            # 切换到性能日志标签
    ...
    def show_download_log_in_panel(self, download_id: Optional[str] = None):
        """在统一面板中显示下载日志"""
        try:
            # 切换到下载日志标签
    ...
    def show_network_log_in_panel(self, request_type: Optional[str] = None):
        """在统一面板中显示网络日志"""
        try:
            # 切换到网络日志标签
            ...
            try:
                with open(log_path, 'r', encoding='utf-8') as file:
                    ...
                    if operation_name:
                        # 过滤出特定操作的日志
                        lines = content.split('\n')
                        operation_lines = [line for line in lines if operation_name in line]
    ...
    def _switch_to_log_tab(self, tab_name: str):
        """切换到指定的日志标签页"""
    ...
    def _apply_server_filter(self, server_name: str):
        """应用服务器过滤"""
    ...
    def _apply_process_filter(self, process_name: str):
        """应用进程过滤"""
        if hasattr(self.log_panel, 'log_filter') and self.log_panel.log_filter:
            filter_config = {
    ...
    def _apply_performance_filter(self, operation_name: str):
        """应用性能过滤"""
        if hasattr(self.log_panel, 'log_filter') and self.log_panel.log_filter:
            filter_config = {
    ...
    def _apply_download_filter(self, download_id: str):
        """应用下载过滤"""
        if hasattr(self.log_panel, 'log_filter') and self.log_panel.log_filter:
            filter_config = {
                'keywords': download_id,
                'case_sensitive': False
            }
            self.log_panel.log_filter.set_filters(filter_config)
    
    def _apply_network_filter(self, request_type: str):
        """应用网络过滤"""
        if hasattr(self.log_panel, 'log_filter') and self.log_panel.log_filter:
            filter_config = {
                'keywords': request_type,
                'case_sensitive': False
            }
            self.log_panel.log_filter.set_filters(filter_config)
    
    def _show_no_log_message(self, source_name: str):
        """显示无日志消息"""
    ...
    def register_custom_log_source(self, name: str, reader: Callable, path_pattern: str):
        """注册自定义日志源"""
...
def get_log_integration_manager(log_panel: Optional[LogPanel] = None) -> Optional[LogIntegrationManager]:
    """获取日志集成管理器实例"""

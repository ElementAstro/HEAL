...
from qfluentwidgets import (
    FluentIcon, PrimaryPushButton, FlyoutViewBase
)
...
class CustomFlyoutViewFiddler(FlyoutViewBase):
    ...
    def init_ui(self):
        ...
        # 连接信号
        self.gc_button.clicked.connect(lambda: self.handle_fiddler_button('grasscutter'))
        self.ht_button.clicked.connect(lambda: self.handle_fiddler_button('hutao'))

    def handle_fiddler_button(self, mode: str):
        """处理Fiddler按钮点击"""
        try:
            # 导入必要的模块
            import subprocess

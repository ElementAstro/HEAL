...
class LazyLoadWorker(QThread):
    ...
    def __init__(self, setting_key: str, loader_func: Callable, parent=None):
...
class LazySettingProxy(Generic[T]):
    ...
    def __init__(self, setting_key: str, loader_func: Callable[[], T], 
    ...
    def _load_sync(self) -> None:
        ...
        try:
            ...
            self._loaded = True
            load_time = time.time() - start_time
            self.logger.debug(f"Synchronously loaded {self.setting_key} in {load_time:.3f}s")
        except Exception as e:
...
class LazySettingsManager(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="lazy_settings")
    ...
    def register_lazy_setting(self, setting_key: str, loader_func: Callable, 
                            fallback_value: Any = None, preload: bool = False) -> LazySettingProxy:
    ...
    def _on_setting_loaded(self, setting_key: str, value: Any):
    ...
    def _on_loading_failed(self, setting_key: str, error: str):
    ...
    def _cleanup_worker(self, setting_key: str):
        """Clean up worker thread"""
        worker = self.workers.pop(setting_key, None)
        if worker:
            worker.deleteLater()
    
    def _preload_next_setting(self):
    ...
    def reload_setting(self, setting_key: str):
...
def get_lazy_manager() -> LazySettingsManager:
    ...
    if _lazy_manager is None:
        ...
        _lazy_manager.register_lazy_setting(
            'config_editor', 
            LazySettings.create_config_editor,
            fallback_value=None
        )
...
def lazy_setting(setting_key: str, fallback_value: Any = None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if value is not None:
                ...
                return func(*args, **kwargs) if fallback_value is None else fallback_value
        return wrapper
    return decorator

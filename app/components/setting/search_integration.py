...
from app.components.setting.search_engine import (
    SettingsSearchEngine, SettingItem, SearchResult, SearchType, FilterType
)
from app.components.setting.search_ui import SettingsSearchWidget
from app.components.setting.performance_manager import get_performance_manager
...
class SettingsSearchIntegrator(QObject):
    ...
    def register_setting_interface(self, interface_name: str, interface: SettingCardGroup):
        ...
        try:
            ...
            self.logger.info(f"Registered {len(settings)} settings from {interface_name}")
            
        except Exception as e:
            self.logger.error(f"Failed to register interface {interface_name}: {e}")
    
    def _extract_settings_from_interface(self, interface: SettingCardGroup, 
                                       category: str) -> List[SettingItem]:
    ...
    def _create_setting_item_from_widget(self, widget: QWidget, category: str) -> Optional[SettingItem]:
    ...
    def register_custom_setting(self, key: str, title: str, description: str,
                              category: str, setting_type: str, value: Any = None,
                              widget: QWidget = None, keywords: List[str] = None):
    ...
    def update_setting_value(self, key: str, new_value: Any):
    ...
    def mark_setting_as_favorite(self, key: str, is_favorite: bool = True):
    ...
    def on_setting_selected(self, setting_item: SettingItem):
    ...
    def get_search_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'categories': len(set(item.category for item in self.settings_registry.values()))
        }
    ...
    def import_favorites(self, favorites: Dict[str, bool]):
    ...
    def rebuild_search_index(self, interfaces: Dict[str, SettingCardGroup]):
...
class SearchEnabledSettingsInterface:
    ...
    def add_search_to_layout(self, layout: QVBoxLayout):
        """Add search widget to the layout"""
        # Insert search widget at the top
        layout.insertWidget(0, self.search_widget)
    
    def register_interfaces_for_search(self, interfaces: Dict[str, SettingCardGroup]):
...
def register_setting_for_search(key: str, title: str, description: str,
                               category: str, setting_type: str, value: Any = None,
                               widget: QWidget = None, keywords: List[str] = None):

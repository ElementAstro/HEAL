...
from PySide6.QtWidgets import (
    ...
    QDateEdit, QSpinBox, QGroupBox, QButtonGroup, QRadioButton
)
from PySide6.QtCore import Qt, Signal, QDate
from qfluentwidgets import (
    CardWidget, BodyLabel, CaptionLabel, CheckBox, ComboBox,
    Slider, DatePicker, SpinBox, ToggleButton, FluentIcon
)

from app.components.setting.search_engine import SearchFilter, FilterType, SettingItem
...
class DateRangeFilter(CardWidget):
    ...
    def on_date_changed(self):
        ...
        def date_filter(item: SettingItem) -> bool:
            return from_timestamp <= item.last_modified <= to_timestamp
...
class ComplexityFilter(CardWidget):
    ...
    def setup_ui(self):
        ...
        desc = CaptionLabel("Filter by setting complexity (basic, intermediate, advanced)")
        desc.setStyleSheet("color: #666;")
        layout.addWidget(desc)
...
class UsageFrequencyFilter(CardWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.logger = get_logger('usage_frequency_filter', module='UsageFrequencyFilter')
    ...
    def update_usage_stats(self, stats: Dict[str, int]):
    ...
    def _parse_expression(self, expression: str) -> Callable[[SettingItem], bool]:
        ...
        def filter_function(item: SettingItem) -> bool:
            try:
                ...
                return (expression in item.title.lower() or 
                       expression in item.description.lower() or
                       expression in item.category.lower())
...
class AdvancedFilterManager(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.logger = get_logger('advanced_filter_manager', module='AdvancedFilterManager')
    ...
    def on_filter_changed(self, advanced_filter: AdvancedFilter):
    ...
    def get_combined_filter_function(self) -> Callable[[SettingItem], bool]:
        ...
        def combined_filter(item: SettingItem) -> bool:
            ...
            return all(f.filter_function(item) for f in self.active_filters if f.enabled)

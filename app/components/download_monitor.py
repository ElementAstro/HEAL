...
from PySide6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout,  # QGroupBox, QLabel, removed as per error
    # QProgressBar, QListWidget, QListWidgetItem removed as per error
)
from PySide6.QtGui import QFont  # QColor removed as per error
from qfluentwidgets import (
    # InfoBarIcon removed as per error
    PrimaryPushButton, PushButton, InfoBar, InfoBarPosition,
    # BodyLabel removed as per error
    ProgressBar, TransparentToolButton, FluentIcon, CardWidget,
    CaptionLabel, StrongBodyLabel, IndeterminateProgressBar, ScrollArea
)
...
class DownloadItemCard(CardWidget):
    ...
    def init_ui(self):
        ...
        self.filename_label = StrongBodyLabel(
            Path(self.download_item.file_path).name)  # Use file_path
    ...
    def update_progress(self, download_item: DownloadItem):
        ...
        if download_item.status == DownloadStatus.DOWNLOADING and download_item.file_size == 0:  # Use file_size
            # 未知大小的下载，使用无限进度条
            self.progress_bar.setVisible(False)
            if not hasattr(self, 'indeterminate_bar'):
                self.indeterminate_bar = IndeterminateProgressBar(self)
                if isinstance(card_layout, QVBoxLayout):  # Check if layout is QVBoxLayout
            ...
            if download_item.file_size > 0:  # Use file_size
                progress = int((download_item.downloaded_size /
                               download_item.file_size) * 100)  # Use file_size
        ...
        self.cancel_btn.setEnabled(download_item.status in [
...
class DownloadManagerWidget(QWidget):
    ...
    def _on_download_status_changed_for_infobar(self, download_id: str, status: DownloadStatus):
    ...
    def add_download_card(self, download_item: DownloadItem):
        """添加下载卡片"""
        if download_item.id in self.download_cards:  # Avoid duplicates if refresh logic changes
    ...
    def cancel_download(self, download_id: str):
        """取消下载"""
        if self.download_manager.cancel_download(download_id):  # Manager will set status to CANCELLED
    ...
    def clear_completed_downloads(self):
        ...
        for download_id, card_widget in list(self.download_cards.items()):
            ...
            if item and item.status in [DownloadStatus.COMPLETED, DownloadStatus.CANCELLED, DownloadStatus.FAILED]:
    ...
    def on_download_started(self, download_id: str):
        """下载开始事件"""
    ...
    def on_download_paused(self, download_id: str):
    ...
    def on_download_resumed(self, download_id: str):
    ...
    def on_download_cancelled(self, download_id: str):

...
class ModuleMetricsManager:
    ...
    def __init__(self):
        self.logger = get_logger('module_metrics_manager', module='ModuleMetricsManager')
    ...
    def record_operation(self, module_name: str, success: bool = True):
        """记录操作"""
        try:
    ...
    def record_error(self, module_name: str, error_message: str):
        """记录错误"""
        try:
            metrics = self.initialize_metrics(module_name)
            metrics.error_count += 1
    ...
    def record_load_time(self, module_name: str, load_time: float):
        """记录加载时间"""
        self.update_metrics(module_name, load_time=load_time)

    def update_resource_usage(self, module_name: str, cpu_usage: float = 0.0, memory_usage: float = 0.0):
        """更新资源使用情况"""
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        avg_success_rate = sum(m.success_rate for m in self.module_metrics.values()) / total_modules if total_modules > 0 else 100.0
        ...
        return {
            ...
            'modules_with_errors': len([m for m in self.module_metrics.values() if m.error_count > 0])
        }

...
from .module_error_handler import ModuleErrorHandler, ErrorCategory, ErrorSeverity, ErrorContext
from .module_notification_system import ModuleNotificationSystem, NotificationType, NotificationPriority
...
class BulkOperation:
    ...
    progress: BulkOperationProgress = None
...
class BulkOperationWorker(QThread):
    ...
    module_completed = Signal(str, str, bool, str)  # operation_id, module_name, success, message
    
    def __init__(self, operation: BulkOperation, operation_handler: Callable, parent=None):
    ...
    def run(self):
        ...
        try:
            ...
            max_workers = min(4, len(self.operation.module_names))  # Limit concurrent operations
            
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                # Submit all module operations
                future_to_module = {
                    executor.submit(self._execute_single_operation, module_name): module_name
                ...
                for future in as_completed(future_to_module):
                    ...
                    try:
                        ...
                        with QMutexLocker(self.mutex):
                            ...
                            self.operation.progress.overall_progress = self.operation.progress.completion_rate
                            self.operation.progress.current_module = module_name
                            
                            # Estimate remaining time
                            elapsed_time = time.time() - self.operation.progress.start_time
                            if self.operation.progress.completed_modules > 0:
                                avg_time_per_module = elapsed_time / self.operation.progress.completed_modules
                                remaining_modules = self.operation.progress.total_modules - self.operation.progress.completed_modules
                                self.operation.progress.estimated_time_remaining = avg_time_per_module * remaining_modules
            ...
            elif self.operation.progress.failed_modules == 0:
                self.operation.status = BulkOperationStatus.COMPLETED
            elif self.operation.progress.successful_modules == 0:
            ...
            overall_success = self.operation.status in [BulkOperationStatus.COMPLETED, BulkOperationStatus.PARTIAL]
...
class ModuleBulkOperations(QObject):
    ...
    operation_completed = Signal(str, bool)  # operation_id, success
    module_operation_completed = Signal(str, str, bool, str)  # operation_id, module_name, success, message
    
    def __init__(self, error_handler: ModuleErrorHandler, 
                 notification_system: ModuleNotificationSystem, parent=None):
        super().__init__(parent)
    ...
    def register_operation_handler(self, operation_type: BulkOperationType, handler: Callable):
    ...
    def set_selected_modules(self, module_names: List[str]):
    ...
    def add_to_selection(self, module_name: str):
        """Add module to selection"""
        self.selected_modules.add(module_name)
    
    def remove_from_selection(self, module_name: str):
    ...
    def execute_bulk_operation(self, 
        ...
        self.logger.info(f"Started bulk operation {operation_id}: {operation_type.value} on {len(module_names)} modules")
        return operation_id
    ...
    def get_completed_operations(self) -> List[BulkOperation]:
        ...
        return [op for op in self.active_operations.values() 
                if op.status in [BulkOperationStatus.COMPLETED, BulkOperationStatus.FAILED, 
                               BulkOperationStatus.CANCELLED, BulkOperationStatus.PARTIAL]]
    ...
    def cleanup_completed_operations(self, older_than_hours: int = 24):
        ...
        for operation_id, operation in self.active_operations.items():
            if (operation.status in [BulkOperationStatus.COMPLETED, BulkOperationStatus.FAILED, 
                                   BulkOperationStatus.CANCELLED, BulkOperationStatus.PARTIAL] and
    ...
    def _on_progress_updated(self, operation_id: str, progress_data: Dict[str, Any]):
        ...
        if operation.notification_id:
            progress_message = f"Processing {progress_data.get('current_module', '...')} " \
    ...
    def _on_module_completed(self, operation_id: str, module_name: str, success: bool, message: str):
        """Handle individual module completion"""
        self.module_operation_completed.emit(operation_id, module_name, success, message)
    ...
    def _on_operation_completed(self, operation_id: str, success: bool):
        ...
        self.logger.info(f"Bulk operation {operation_id} completed with status: {operation.status.value}")

...
from .module_models import ModuleState, ModuleConfig
...
class ModuleWorkflowManager(QObject):
    ...
    def register_step_handler(self, step: WorkflowStep, handler: Callable, rollback_handler: Optional[Callable] = None):
    ...
    def start_workflow(self, module_name: str, workflow_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
    ...
    def execute_next_step(self, workflow_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self._fail_workflow_step(workflow_id, current_step, "Step handler returned False")
                return False
    ...
    def update_step_progress(self, workflow_id: str, progress: float, message: str = ""):
    ...
    def rollback_workflow(self, workflow_id: str, to_step: Optional[WorkflowStep] = None) -> bool:
        ...
        try:
            ...
            for step in steps_to_rollback:
                if step in self.rollback_handlers:
                    try:
                        ...
                        rollback_handler(workflow_id, workflow.module_name, workflow.steps[step])
            ...
            self.logger.info(f"Rolled back workflow {workflow_id} to step {to_step.value}")
            return True
    ...
    def get_workflows_for_module(self, module_name: str) -> List[ModuleWorkflow]:
        ...
        return [w for w in self.active_workflows.values() if w.module_name == module_name]
    ...
    def _fail_workflow_step(self, workflow_id: str, step: WorkflowStep, error: str):
    ...
    def _update_overall_progress(self, workflow_id: str):
        """Update overall workflow progress"""
        workflow = self.active_workflows[workflow_id]
        ...
        overall_progress = (completed_steps + current_step_progress) / total_steps * 100.0

...
from PySide6.QtWidgets import (
    ...
    QFrame, QScrollArea, QSplitter, QPushButton
)
from PySide6.QtGui import QFont, QPixmap
from qfluentwidgets import (
    FluentIcon, TitleLabel, SubtitleLabel, BodyLabel, CaptionLabel,
    PushButton, ProgressBar, InfoBar, InfoBarPosition, CardWidget,
...
class QuickActionCard(SimpleCardWidget):
    ...
    def __init__(self, title: str, description: str, icon: FluentIcon, 
                 action_id: str, parent=None):
        super().__init__(parent)
        self.action_id = action_id
        self.setup_ui(title, description, icon)
    
    def setup_ui(self, title: str, description: str, icon: FluentIcon):
        """Setup the card UI"""
        ...
        action_button.clicked.connect(lambda: self.action_triggered.emit(self.action_id))
...
class SystemHealthCard(ElevatedCardWidget):
    ...
    def setup_ui(self):
        ...
        self.health_score_label.setStyleSheet("color: #10b981; font-size: 24px; font-weight: bold;")
    ...
    def update_health_data(self, stats: Optional[DashboardStats] = None):
        ...
        self.health_score_label.setStyleSheet(f"color: {color}; font-size: 24px; font-weight: bold;")
...
class RecentActivityCard(ElevatedCardWidget):
    ...
    def add_activity(self, title: str, description: str, activity_type: str = "info"):
...
class ModuleDashboard(QWidget):
    ...
    def setup_ui(self):
        ...
        quick_actions = [
            ...
            ("系统检查", "检查系统健康状态", FluentIcon.CARE_RIGHT_SOLID, "system_check"),
        ...
        # Module statistics
        self.total_modules_card = self.create_stat_card("总模组数", "0", FluentIcon.FOLDER)
        self.enabled_modules_card = self.create_stat_card("已启用", "0", FluentIcon.ACCEPT)
        self.error_modules_card = self.create_stat_card("错误模组", "0", FluentIcon.CLOSE)
    ...
    def create_stat_card(self, title: str, value: str, icon: FluentIcon) -> SimpleCardWidget:
    ...
    def _on_workflow_completed(self, workflow_id: str):
        ...
        if workflow:
            self.add_activity(
                "工作流完成",
                f"模组 '{workflow.module_name}' 工作流已完成",
                "success"
            )
    
    def _on_workflow_failed(self, workflow_id: str, step: str, error: str):
        """Handle workflow failure"""
        workflow = self.workflow_manager.get_workflow(workflow_id)
        if workflow:
            self.add_activity(
                "工作流失败",
                f"模组 '{workflow.module_name}' 在{step}步骤失败",
                "error"
            )
    ...
    def _on_error_occurred(self, error_id: str):

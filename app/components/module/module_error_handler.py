...
class ModuleErrorHandler(QObject):
    ...
    def register_recovery_handler(self, action_id: str, handler: Callable):
    ...
    def handle_error(self, 
        ...
        error.recovery_actions = self._generate_recovery_actions(exception, category, context)
        ...
        # Log error
        self.logger.error(f"Error {error_id}: {error_message}", 
    ...
    def handle_validation_error(self, validation_issues: List[Dict[str, Any]], 
                              module_name: str) -> str:
        ...
        if critical_issues:
            ...
            message = f"Module validation failed with {len(critical_issues)} critical issues"
        else:
    ...
    def execute_recovery_action(self, error_id: str, action_id: str) -> bool:
        ...
        if not recovery_action:
            self.logger.error(f"Recovery action {action_id} not found for error {error_id}")
            return False
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(f"Recovery action {action_id} executed successfully for error {error_id}")
                return True
            else:
                self.logger.warning(f"Recovery action {action_id} failed for error {error_id}")
                return False
    ...
    def get_recent_errors(self, hours: int = 24) -> List[ModuleError]:
        ...
        return [error for error in self.errors.values() if error.timestamp > cutoff_time]

    def clear_resolved_errors(self):
        """Clear all resolved errors"""
        resolved_count = len([e for e in self.errors.values() if e.resolved])
        self.errors = {eid: error for eid, error in self.errors.items() if not error.resolved}
        self.logger.info(f"Cleared {resolved_count} resolved errors")
        self.save_errors()

    def export_error_report(self, filepath: str, include_resolved: bool = False) -> bool:
        """Export error report to file"""
        try:
            errors_to_export = self.errors.values()

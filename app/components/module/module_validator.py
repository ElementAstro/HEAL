...
class ModuleValidationReport:
    ...
    def is_valid(self) -> bool:
        """模块是否有效"""
        return self.overall_result in [ValidationResult.PASSED, ValidationResult.WARNING]

    @property
    def critical_issues(self) -> List[ValidationIssue]:
        """获取严重问题"""
        return [issue for issue in self.issues if issue.level == ValidationResult.CRITICAL]

    @property
    def error_issues(self) -> List[ValidationIssue]:
        """获取错误问题"""
        return [issue for issue in self.issues if issue.level == ValidationResult.FAILED]
...
class ModuleValidator:
    ...
    def _check_executable_files(self, zip_file: zipfile.ZipFile) -> List[ValidationIssue]:
        """检查可执行文件"""
        issues = []
        file_list = zip_file.namelist()

        executable_files = [f for f in file_list if f.endswith(
            ('.exe', '.bat', '.sh', '.cmd'))]
        if executable_files:
            issues.append(ValidationIssue(
                level=ValidationResult.WARNING,
    ...
    def _find_metadata(self, path: Path, issues: List[ValidationIssue]) -> Dict[str, Any]:
        """查找元数据文件"""
        metadata = {}
    ...
    def _extract_metadata_from_archive(self, path: Path, metadata_files: List[str],
    ...
    def _validate_required_fields(self, metadata: Dict[str, Any]) -> List[ValidationIssue]:
        """验证必需字段"""
        issues = []

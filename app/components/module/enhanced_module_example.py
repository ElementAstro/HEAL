...
from .module_notification_system import ModuleNotificationSystem, NotificationType, NotificationAction
...
class EnhancedModuleInterface(QMainWindow):
    ...
    def _integrate_module_manager(self):
        ...
        try:
            if action_id == "refresh_modules":
                ...
                if all_modules:
                    ...
                    self.notification_system.show_warning("无模组", "没有找到可验证的模组")
    ...
    def perform_system_check(self):
        ...
        try:
            ...
            stuck_workflows = [w for w in active_workflows.values() 
                             if w.updated_at < (time.time() - 3600)]  # 1 hour old
            if stuck_workflows:
                issues_found += len(stuck_workflows)
            
            # Check module states
            error_modules = [name for name, info in self.module_manager.mods.items() 
                           if not info.usable]
            if error_modules:
    ...
    def export_system_report(self):
        ...
        try:
            ...
            self.error_handler.export_error_report(error_report_file, include_resolved=True)
    ...
    def handle_download_step(self, workflow_id: str, module_name: str, step_info) -> bool:
    ...
    def handle_validate_step(self, workflow_id: str, module_name: str, step_info) -> bool:
    ...
    def handle_install_step(self, workflow_id: str, module_name: str, step_info) -> bool:
    ...
    def handle_configure_step(self, workflow_id: str, module_name: str, step_info) -> bool:
...
def main():
    ...
    interface.notification_system.show_info(
        "欢迎使用", "增强型模组管理系统已启动"
    )

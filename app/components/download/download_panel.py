...
from qfluentwidgets import (
    ...
    ProgressBar, InfoBar, InfoBarPosition, ScrollArea,
    Pivot, SegmentedWidget
)

from app.common.logging_config import get_logger
from app.common.i18n import t
from app.components.download_monitor import DownloadManagerWidget, DownloadItemCard
from app.model.download_manager import DownloadItem, DownloadStatus, DownloadManager
...
class QuickDownloadCard(CardWidget):
    ...
    def init_ui(self):
        ...
        self.git_btn.clicked.connect(lambda: self.download_requested.emit("Git", "https://git-scm.com/download/win"))
        
        self.python_btn = PushButton("Python")
        self.python_btn.setIcon(FluentIcon.DEVELOPER_TOOLS)
        self.python_btn.clicked.connect(lambda: self.download_requested.emit("Python", "https://www.python.org/downloads/"))
        
        self.nodejs_btn = PushButton("Node.js")
        self.nodejs_btn.setIcon(FluentIcon.GLOBE)
        self.nodejs_btn.clicked.connect(lambda: self.download_requested.emit("Node.js", "https://nodejs.org/en/download/"))
...
class DownloadPanel(QWidget):
    ...
    def init_ui(self):
        ...
        # 连接信号
        self.download_monitor.download_completed_signal.connect(self.on_download_completed)
    ...
    def on_download_completed(self, download_item):
        ...
        history_item = {
            ...
            'status': 'completed' if download_item.status == DownloadStatus.COMPLETED else 'failed',

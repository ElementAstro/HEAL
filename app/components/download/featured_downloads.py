...
from qfluentwidgets import (
    ...
    InfoBadge, InfoLevel, HyperlinkButton
)
...
class FeaturedDownloadCard(CardWidget):
    ...
    def __init__(self, item_data: Dict[str, Any], parent=None):
        ...
        self.logger = get_logger('featured_download_card', module='FeaturedDownloadCard')
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Description
        description = self.item_data.get('content', self.item_data.get('description', ''))
        if description:
...
class FeaturedDownloadsSection(CardWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.logger = get_logger('featured_downloads_section', module='FeaturedDownloadsSection')
        self.featured_items = []
        self.init_ui()
    ...
    def _load_default_featured_items(self):
        """Load default featured items"""
        default_items = [
            {
                'title': 'Git',
    ...
    def set_featured_items(self, items: List[Dict[str, Any]]):
        """Set featured items"""
    ...
    def add_featured_item(self, item: Dict[str, Any]):
        """Add a new featured item"""
        self.featured_items.append(item)
        self._refresh_cards()
        
    def remove_featured_item(self, title: str):
        """Remove a featured item by title"""
        self.featured_items = [item for item in self.featured_items if item.get('title') != title]
        self._refresh_cards()
        
    def update_featured_item(self, title: str, updated_data: Dict[str, Any]):

...
from PySide6.QtWidgets import QWidget, QMessageBox, QHBoxLayout, QVBoxLayout, QCompleter
from PySide6.QtCore import QObject, Signal, QTimer, QStringListModel, Qt
from qfluentwidgets import (
    LineEdit, PushButton, ComboBox, SearchLineEdit,
    ToolButton, FluentIcon, InfoBar, InfoBarPosition,
...
class DownloadSearchManager(QObject):
    ...
    def _search_in_data(self, search_text: str) -> List[Dict[str, Any]]:
        ...
        for section_title, items in self.search_index.items():
            ...
            # 搜索项目
            for item in items:
                if (search_text in item['title'].lower() or
                    search_text in item['content'].lower() or
    ...
    def perform_search(self, search_text: str, category: str = "all"):
        """执行搜索（新增方法用于新界面）"""
        ...
        self.search_performed.emit(search_text)
        self.logger.info(f"执行搜索: '{search_text}' in '{category}', 找到 {len(results)} 个结果")

    def _search_in_category(self, search_text: str, category_id: str) -> List[Dict[str, Any]]:
        """在指定分类中搜索"""
        results = []

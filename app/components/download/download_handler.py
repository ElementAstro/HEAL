...
class DownloadHandler(QObject):
    ...
    def generate_download_url(self, types: str, repo_url: str, mirror_url: Optional[str], 
                            mirror_branch: Optional[str] = None, is_add: bool = True) -> str:
        """生成下载URL"""
        from pathlib import Path
        from app.model.config import cfg
    ...
    def _execute_download(self, name: str, types: str, command: str, file_path: str):
        """执行下载"""
    ...
    def _handle_existing_file(self, file_path: str):
        """处理已存在的文件"""
    ...
    def _show_file_exists_message(self):
        """显示文件已存在消息"""
        InfoBar.error(

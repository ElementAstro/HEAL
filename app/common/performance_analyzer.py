...
class FunctionProfile:
    ...
    def add_call(self, execution_time: float, memory_usage: Optional[float] = None):
        """添加函数调用记录"""
        self.call_count += 1
...
class PerformanceAnalyzer:
    ...
    def profile_function(self, threshold: float = None):
        """函数性能分析装饰器"""
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    with self.analysis_lock:
                        ...
                        self.function_profiles[func_name].add_call(execution_time, memory_delta)
                    
                    # 检查性能问题
                    used_threshold = threshold or self.thresholds['function_execution_time']
                    ...
                    if memory_delta > self.thresholds['memory_usage_mb']:
                        self._report_performance_issue(
                            category='memory',
                            severity=self._get_severity(memory_delta, self.thresholds['memory_usage_mb']),
    ...
    @contextmanager
    def profile_io_operation(self, operation_name: str):
        """IO操作性能分析上下文管理器"""
        start_time = time.perf_counter()
        io_info = {
        ...
        try:
            ...
            if execution_time > self.thresholds['io_operation_time']:
                self._report_performance_issue(
                    category='io',
                    severity=self._get_severity(execution_time, self.thresholds['io_operation_time']),
    ...
    def get_memory_heavy_functions(self, limit: int = 10) -> List[FunctionProfile]:
        ...
        with self.analysis_lock:
            memory_functions = [
                func for func in self.function_profiles.values()
                if func.memory_usage
            ]
            
            sorted_functions = sorted(
                memory_functions,
                key=lambda x: sum(x.memory_usage) / len(x.memory_usage) if x.memory_usage else 0,
                reverse=True
            )
            return sorted_functions[:limit]
        ...
        with self.analysis_lock:
            return {
                ...
                'io_analysis': {
                    ...
                    'avg_time': sum(op['execution_time'] for op in self.io_operations) / len(self.io_operations) if self.io_operations else 0,
                    'slow_operations': [
                        op for op in self.io_operations
                        if op['execution_time'] > self.thresholds['io_operation_time']
                    ]
                }
            }
    
    def _report_performance_issue(self, **kwargs):
        """报告性能问题"""
...
# 便捷装饰器
def profile_performance(threshold: float = None):
    """性能分析装饰器"""
    return global_performance_analyzer.profile_function(threshold)


def profile_io(operation_name: str):
    """IO性能分析上下文管理器"""
...
class PerformanceOptimizer:
    ...
    def analyze_and_suggest(self) -> Dict[str, List[str]]:
        ...
        # 分析IO操作
        slow_io_ops = [
            op for op in self.analyzer.io_operations
    ...
    def _estimate_optimization_impact(self, issues: List[PerformanceIssue]) -> Dict[str, str]:
        """估算优化影响"""
        if not issues:
            return {'overall': 'low'}
        ...
        if critical_count > 0:
            return {'overall': 'high', 'description': '存在关键性能问题，优化后将显著提升性能'}
        elif high_count > 2:
            return {'overall': 'medium', 'description': '存在多个高优先级问题，优化后将明显改善性能'}
        else:
            return {'overall': 'low', 'description': '性能问题较少，优化后将适度提升性能'}

    def _suggest_implementation_order(self, suggestions: Dict[str, List[str]]) -> List[str]:
        """建议实现顺序"""
        order = []

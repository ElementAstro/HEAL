...
class ValidationResult:
    ...
    def add_fixed(self, message: str):
...
class ConfigValidator:
    ...
    def _load_schemas(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "config.json": {
                "type": "object",
                "required": ["PASSWORD", "UID", "KEY", "SERVER_URL", "PROXY_PORT", "SERVER"],
                "properties": {
    ...
    def _create_default_config(self, file_path: str, result: ValidationResult):
        """创建默认配置文件"""
        try:
            filename = os.path.basename(file_path)
            ...
            if filename in self.default_values:
                ...
                with open(file_path, 'w', encoding='utf-8') as f:
                    json.dump(self.default_values[filename], f, indent=2, ensure_ascii=False)
    ...
    def _fix_schema_issues(self, config_data: Dict[str, Any], filename: str, result: ValidationResult):
        """修复模式问题"""
        try:
            if filename in self.default_values:
                ...
                # 递归修复缺失的字段
                def fix_missing_fields(data: Dict[str, Any], defaults: Dict[str, Any], path: str = ""):
                    ...
                    for key, default_value in defaults.items():
                        ...
                        if key not in data:
                            ...
                            result.add_fixed(f"添加缺失字段 {current_path}: {default_value}")
                            fixed_count += 1
    ...
    def _validate_custom_rules(self, config_data: Dict[str, Any], filename: str, 
                              result: ValidationResult, auto_fix: bool):
        """自定义验证规则"""
                ...
                for server_name, server_config in config_data["SERVER"].items():
                    ...
                    for field in required_fields:
                        if field not in server_config:
                            result.add_error(f"服务器 {server_name} 缺少必需字段: {field}")
    
    def _validate_security(self, config_data: Dict[str, Any], filename: str, result: ValidationResult):
        """安全性验证"""
        if filename == "config.json":
            # 检查默认密码
    ...
    def validate_all_configs(self, config_dir: str = "config", auto_fix: bool = False) -> Dict[str, ValidationResult]:
        """验证所有配置文件"""
        results = {}

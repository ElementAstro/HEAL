...
class ResourceInfo:
    ...
    def __init__(self, resource_id: str, resource_type: str, 
                 resource_obj: Any, cleanup_func: Callable[[], None],
                 description: str = ""):
        self.resource_id = resource_id
        self.resource_type = resource_type
        self.resource_obj = weakref.ref(resource_obj) if hasattr(resource_obj, '__weakref__') else resource_obj
        ...
        try:
            with self._lock:
                ...
                try:
                    ...
                    self.logger.debug(f"清理资源: {resource_id} ({resource_info.resource_type})")
                    self.resource_cleaned.emit(resource_id, resource_info.resource_type)
                    return True
                    
                except Exception as cleanup_error:
                    self.logger.error(f"清理资源 {resource_id} 时发生错误: {cleanup_error}")
                    return False
    ...
    def get_resource_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                cleaned_resources = sum(1 for info in self.resources.values() if info.is_cleaned)
...
def register_custom_resource(resource_id: str, resource_obj: Any, 

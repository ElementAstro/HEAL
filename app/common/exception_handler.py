...
class ExceptionInfo:
    ...
    def _generate_user_message(self) -> str:
        """生成用户友好的错误消息"""
        error_messages = {
            ExceptionType.NETWORK_ERROR: tr("network_error", "网络连接出现问题，请检查网络设置"),
            ExceptionType.FILE_ERROR: tr("file_error", "文件操作失败，请检查文件权限和磁盘空间"),
            ExceptionType.PROCESS_ERROR: tr("process_error", "进程启动或管理失败，请检查系统资源"),
            ExceptionType.DOWNLOAD_ERROR: tr("download_error", "下载失败，请检查网络连接和存储空间"),
            ExceptionType.CONFIG_ERROR: tr("config_error", "配置文件错误，请检查配置设置"),
            ExceptionType.PERMISSION_ERROR: tr("permission_error", "权限不足，请以管理员身份运行"),
            ExceptionType.VALIDATION_ERROR: tr("validation_error", "输入数据无效，请检查输入内容"),
            ExceptionType.UNKNOWN_ERROR: tr("unknown_error", "发生未知错误，请查看详细信息")
        }
        ...
        try:
            return {
                ...
                "disk_usage": {
                    ...
                    for path, usage in [(p.mountpoint, psutil.disk_usage(p.mountpoint))
                                        for p in psutil.disk_partitions() if p.fstype][:3]
                },
    ...
    def _generate_error_hash(self) -> str:
        ...
        return hashlib.md5(error_signature.encode()).hexdigest()  # Use full MD5 hash for better uniqueness
...
class ExceptionHandler(QObject):
    ...
    def _analyze_exception(self, exception: Exception, traceback_obj=None) -> ExceptionInfo:
        """分析异常并确定类型和严重性"""
        ...
        elif any(keyword in exc_str for keyword in ['file', 'directory', 'path', 'permission denied']):
        ...
        elif any(keyword in exc_str for keyword in ['process', 'subprocess', 'command']):
        ...
        elif any(keyword in exc_str for keyword in ['config', 'configuration', 'setting']):
        ...
        return ExceptionInfo(
            ...
            severity=severity,
            recovery_suggestions=recovery_suggestions,
            auto_retry=exc_type in [
                ExceptionType.NETWORK_ERROR, ExceptionType.DOWNLOAD_ERROR]
        )
    ...
    def _log_exception(self, exception_info: ExceptionInfo):
        ...
        # 统计错误次数
        self.error_counts[exception_info.exc_type] = self.error_counts.get(
            exception_info.exc_type, 0) + 1

        # 限制历史记录大小
        if len(self.error_history) > 1000:
            self.error_history = self.error_history[-500:]

    def _show_user_notification(self, exception_info: ExceptionInfo, parent_widget: Optional[QWidget] = None):
        """显示用户通知"""
        ...
        info_bar = InfoBar(
            ...
            content=exception_info.technical_details[:100] + "..." if len(
                exception_info.technical_details) > 100 else exception_info.technical_details,
    ...
    def _show_message_box(self, exception_info: ExceptionInfo):
        """显示消息框"""
        msg_box = QMessageBox()
    ...
    def _show_recovery_dialog(self, exception_info: ExceptionInfo, parent_widget: QWidget):
        """显示恢复建议对话框"""
        ...
        if exception_info.auto_retry and exception_info.retry_count < exception_info.max_retries:
    ...
    def _attempt_recovery(self, exception_info: ExceptionInfo):
        """尝试异常恢复"""
...
# 常用异常处理装饰器
def network_exception_handler(func): return exception_handler(
    exc_type=ExceptionType.NETWORK_ERROR,
    severity=ExceptionSeverity.MEDIUM,
    user_message="网络操作失败",
...
def file_exception_handler(func): return exception_handler(
    exc_type=ExceptionType.FILE_ERROR,
    severity=ExceptionSeverity.MEDIUM,
    user_message="文件操作失败",
...
def process_exception_handler(func): return exception_handler(
    exc_type=ExceptionType.PROCESS_ERROR,
    severity=ExceptionSeverity.HIGH,
    user_message="进程操作失败",
...
def download_exception_handler(func): return exception_handler(
    exc_type=ExceptionType.DOWNLOAD_ERROR,
    severity=ExceptionSeverity.MEDIUM,
    user_message="下载失败",
...
# 兼容旧版异常处理器
def exception_handler_legacy(log: str, *default):
...
def exception_traceback_handler_legacy(log: str, *default):
    """ decorator for exception handling (legacy compatibility)

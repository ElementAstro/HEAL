...
from PySide6.QtWidgets import QWidget, QLabel, QPushButton, QLineEdit, QTextEdit
from PySide6.QtCore import QObject, Signal, QTimer
from qfluentwidgets import (
    PushButton, PrimaryPushButton, TitleLabel, SubtitleLabel, 
    BodyLabel, StrongBodyLabel, LineEdit, TextEdit
)

from app.common.i18n import translation_manager, Language, tr
...
class I18nWidget(QObject):
    ...
    def register_element(self, element: QWidget, key: str, 
                        property_name: str = "text", **kwargs):
        """注册需要国际化的UI元素"""
    ...
    def register_text(self, element: Union[QLabel, QPushButton, PushButton], 
                     key: str, **kwargs):
        """注册文本元素（标签、按钮等）"""
        self.register_element(element, key, "setText", **kwargs)
        
    def register_placeholder(self, element: Union[QLineEdit, LineEdit, TextEdit], 
                           key: str, **kwargs):
        """注册占位符文本"""
        self.register_element(element, key, "setPlaceholderText", **kwargs)
        
    def register_window_title(self, key: str, **kwargs):
        """注册窗口标题"""
    ...
    def _apply_translation(self, element_id: str):
        """应用翻译到指定元素"""
        try:
...
class I18nHelper:
    ...
    @staticmethod
    def create_translated_primary_button(key: str, icon=None, **kwargs) -> PrimaryPushButton:
        """创建已翻译的主要按钮"""
        text = tr(key, **kwargs)
        if icon:
...
def tr_button(key: str, icon=None, primary: bool = False, **kwargs) -> Union[PushButton, PrimaryPushButton]:
    """创建翻译按钮的便捷函数"""
    if primary:

...
from app.common.exception_handler import file_exception_handler
from app.common.performance_analyzer import profile_performance, profile_io
from app.common.cache_manager import cached, get_file_cached
...
class JsonUtils:
    ...
    def load_json_file(file_path: Union[str, Path],
                      ...
                      default_content: Optional[Dict[str, Any]] = None) -> JsonLoadResult:
        """
        安全地加载JSON文件
    ...
    def get_json_value(file_path: Union[str, Path],
                      key: str,
    ...
    @staticmethod
    def update_json_value(file_path: Union[str, Path],
                         key: str,
                         value: Any,
        ...
        """
        # 加载现有数据
        result = JsonUtils.load_json_file(file_path, encoding, create_if_missing=True, default_content={})
    ...
    @staticmethod
    def merge_json_files(source_path: Union[str, Path],
                        target_path: Union[str, Path],
                        encoding: str = 'utf-8',
                        deep_merge: bool = True) -> bool:
        """
        合并两个JSON文件
        ...
        # 加载目标文件
        target_result = JsonUtils.load_json_file(target_path, encoding, create_if_missing=True, default_content={})
...
# 便捷函数
def load_config_json(file_path: Union[str, Path], 
                    default_config: Optional[Dict[str, Any]] = None) -> JsonLoadResult:
    """加载配置JSON文件的便捷函数"""
    return JsonUtils.load_json_file(
        file_path, 
        create_if_missing=True, 
        default_content=default_config or {}
    )


def save_config_json(file_path: Union[str, Path], config: Dict[str, Any]) -> bool:
    """保存配置JSON文件的便捷函数"""
    return JsonUtils.save_json_file(file_path, config)


def get_config_value(file_path: Union[str, Path], key: str, default_value: Any = None) -> Any:
    """从配置文件获取值的便捷函数"""
...
def get_json(file_path: Union[str, Path], key: str) -> Any:
    ...
    try:
        # 使用配置验证器验证文件
        from app.common.config_validator import validate_config_file

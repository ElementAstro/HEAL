...
class SignalConnection:
    ...
    def get_source_obj(self):
        """获取源对象"""
        if isinstance(self.source_obj, weakref.ref):
...
class SignalManager(QObject):
    ...
    def enable_debug(self, track_calls: bool = True):
        """启用调试模式"""
    ...
    def connect_signal(self, 
        ...
        try:
            ...
            if self.debug_mode:
                ...
                def debug_wrapper(*args, **kwargs):
                    ...
                    if self.debug_mode:
                        ...
                        self.logger.debug(f"信号触发: {source_obj}.{signal_name} -> {target}.{slot_name}")
            ...
            self.logger.debug(f"信号连接成功: {connection_id} ({source_obj}.{signal_name} -> {target}.{slot_name})")
    ...
    def batch_connect(self, connections: List[Dict[str, Any]]) -> List[str]:
        ...
        self.logger.info(f"批量信号连接完成: {len(connection_ids)}/{len(connections)} 成功")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        valid_connections = sum(1 for conn in self.connections.values() if conn.is_valid())
    ...
    def disconnect_all(self) -> int:
        ...
        self.logger.info(f"断开所有连接完成: {success_count}/{len(connection_ids)} 成功")
...
# 便捷函数
def connect_signal(source_obj: QObject,
                  signal_name: str,
                  target: Union[QObject, Callable],
                  slot_name: Optional[str] = None,
                  connection_type: ConnectionType = ConnectionType.AUTO) -> str:
    """连接信号的便捷函数"""
...
def enable_signal_debug():
    """启用信号调试"""
    global_signal_manager.enable_debug()


def disable_signal_debug():
    """禁用信号调试"""

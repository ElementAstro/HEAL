...
from PySide6.QtWidgets import (
    ...
    QSplitter, QScrollArea, QFrame, QLabel, QPushButton
)
from PySide6.QtCore import Qt, QObject, Signal, QTimer
from qfluentwidgets import (
    FluentIcon, SettingCard, ExpandLayout, ScrollArea,
    PrimaryPushButton, PushButton, InfoBar, InfoBarPosition
)

from app.common.logging_config import get_logger
from app.common.resource_manager import register_timer, register_custom_resource
...
class UIComponentManager(QObject):
    ...
    def create_layout(self, layout_type: str, 
                     name: str,
    ...
    def create_splitter(self, name: str,
                       orientation: Qt.Orientation = Qt.Orientation.Horizontal,
                       stretch_factors: Optional[List[int]] = None) -> QSplitter:
        """
        创建分割器
    ...
    def create_scroll_area(self, name: str,
    ...
    def connect_signal(self, source_obj: QObject,
                      signal_name: str,
        ...
        try:
            ...
            if callable(target_obj):
                ...
                target_desc = target_obj.__name__ if hasattr(target_obj, '__name__') else str(target_obj)
            else:
                # 连接到对象的槽
            ...
            self.signal_connected.emit(connection_info['source'], connection_info['target'])
            self.logger.debug(f"信号连接成功: {connection_info['source']} -> {connection_info['target']}")
    ...
    def show_info_bar(self, title: str,
                     content: str,
                     duration: int = 3000,
                     position: InfoBarPosition = InfoBarPosition.TOP,
                     parent: Optional[QWidget] = None) -> InfoBar:
        """
        显示信息条
...
# 便捷函数
def create_standard_layout(parent: QWidget,
                          layout_type: str = 'vbox',
                          spacing: int = 10,
                          margins: Union[int, List[int]] = 10) -> object:
    """创建标准布局的便捷函数"""
...
def create_standard_splitter(orientation: Qt.Orientation = Qt.Orientation.Horizontal,
                           stretch_factors: Optional[List[int]] = None) -> QSplitter:
    """创建标准分割器的便捷函数"""

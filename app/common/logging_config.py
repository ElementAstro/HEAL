...
class LogMetrics:
    ...
    def record_log(self, level: str, module: str = "unknown"):
        with self.lock:
        ...
        with self.lock:
            ...
            return {
                ...
                'performance_summary': {
                    op: {
                        'count': len(data),
                        'avg_duration': sum(d['duration'] for d in data) / len(data) if data else 0,
...
class LoggingConfig:
    ...
    def _create_custom_format(self, include_correlation: bool = True) -> str:
        """创建自定义日志格式"""
        base_format = "{time:YYYY-MM-DD HH:mm:ss.SSS} | {level: <8} | {name}:{function}:{line}"
        
        if include_correlation:
            # 添加correlation ID和session信息
            correlation_format = " | CID:{extra[correlation_id]} | SID:{extra[session_id]}"
    ...
    def _correlation_filter(self, record):
        ...
        record["extra"]["correlation_id"] = correlation_id.get("")[:8] if correlation_id.get() else "--------"
        record["extra"]["session_id"] = user_session.get("")[:8] if user_session.get() else "--------"
        return True
    
    def _metrics_filter(self, record):
        """记录日志统计信息"""
    ...
    def _alerting_filter(self, record):
        """触发告警回调"""
        level = record["level"].name if hasattr(record["level"], "name") else "INFO"
    ...
    def _combined_filter(self, record, extra_key: Optional[str] = None):
        """组合过滤器"""
        # 添加基础信息
    ...
    def add_module_logger(self, module_name: str, level: str = "DEBUG") -> int:
        ...
        handler = logger.add(
            ...
            filter=lambda record: (
                record["extra"].get("module") == module_name and
                self._combined_filter(record)
            ),
            backtrace=True,
            diagnose=True,
            encoding="utf-8",
    ...
    def set_dynamic_level(self, module_name: str, level: str):
        """动态设置模块日志级别"""
        with self.config_lock:
            self.dynamic_levels[module_name] = level
            logger.info(f"模块 {module_name} 日志级别已设置为 {level}")

    def add_alerting_callback(self, callback: Callable):
        """添加告警回调函数"""
        ...
        try:
            ...
            for log_file in self.log_dir.glob("*.log*"):
                if log_file.stat().st_mtime < cutoff_date.timestamp():
                    ...
                    archive_path = archive_dir / f"{cutoff_date.strftime('%Y%m%d')}_{log_file.name}"
    ...
    def cleanup_logs(self, max_size_mb: int = 100):
        ...
        try:
            ...
            if total_size_mb > max_size_mb:
                # 删除最旧的文件直到满足大小限制
                files = [(f, f.stat().st_mtime) for f in self.log_dir.rglob("*") if f.is_file()]
...
def with_correlation_id(cid: Optional[str] = None):
    ...
    class CorrelationContext:
        def __init__(self, correlation_id_value):
...
def log_exception(exception: Exception, message: Optional[str] = None, **kwargs):
    """记录异常日志"""
...
def archive_logs(days_old: int = 30):
    """归档旧日志"""
    global _logging_config
    if _logging_config is None:
        setup_logging()
...
def cleanup_logs(max_size_mb: int = 100):
    """清理日志文件"""
    global _logging_config
    if _logging_config is None:
        setup_logging()
...
# 向后兼容的函数
def log_performance_legacy(func_name: str, duration: float, **kwargs):
    """记录性能日志 - 向后兼容版本"""
    global _logging_config
    if _logging_config is None:
        setup_logging()

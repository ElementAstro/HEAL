...
class UpdateThread(QThread):
    ...
    def run(self) -> None:
        ...
        if not os.path.exists('firefly-launcher.py'):
            self.logger.debug("'firefly-launcher.py' does not exist. Proceeding with version check.")
            latest_version = get_latest_version()
            installed_version = get_json('./config/version.json', 'APP_VERSION')
            self.logger.debug(f"Installed version: {installed_version}, Latest version: {latest_version}")
            ...
            if latest_version and installed_version:
                if latest_version > installed_version:
                    ...
                    self.logger.warning("Installed version is newer than the latest version available.")
...
def handle_update(status: int, message: str):
    if status == 2:

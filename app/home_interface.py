...
from app.components.home import (
    ...
    ServerManager, DialogManager, ServerStatusCard, StatusOverviewWidget,
    QuickActionBar, CompactBannerWidget
)
...
class Home(QWidget):
    ...
    def _init_optimized_widgets(self):
        ...
        self.server_status_cards = self.layout_manager.create_server_status_cards(cfg.SERVER)
    ...
    def _connect_optimized_signals(self):
        ...
        if self.quick_action_bar:
            ...
            self.quick_action_bar.refresh_requested.connect(self._handle_refresh_request)
            self.quick_action_bar.view_logs_requested.connect(self._handle_view_logs_request)
            self.quick_action_bar.settings_requested.connect(self._handle_settings_request)
    ...
    def _connect_legacy_signals(self):
        ...
        if self.flipView:
            ...
            # 注册定时器到资源管理器
            register_timer(f"home_scroll_timer_{id(self)}", self.scrollTimer,
                          "首页滚动定时器")
    ...
    def handleServerLaunch(self):
        ...
        with with_correlation_id() as cid:
            ...
            if launched_servers_this_run:
                logger.info(f"成功启动 {len(launched_servers_this_run)} 个服务器: {launched_servers_this_run}")
    ...
    def _update_optimized_status_displays(self):
        ...
        if self.status_overview:
            for server_name in cfg.SERVER.keys():
                status = "running" if server_name in self.launched_servers else "stopped"
    ...
    def _handle_server_start_request(self, server_name: str):
        ...
        if self.server_manager.launch_single_server(type('MockButton', (), {'objectName': lambda: server_name, 'setChecked': lambda x: None})()):
            self.launched_servers.append(server_name)
            self._update_optimized_status_displays()

    def _handle_server_stop_request(self, server_name: str):
    ...
    def _handle_server_restart_request(self, server_name: str):
        """Handle server restart request from status card"""
        logger.info(f"Restart request for server: {server_name}")
        self.server_manager.restart_server(server_name)

    def _handle_server_settings_request(self, server_name: str):
    ...
    def _handle_view_logs_request(self):

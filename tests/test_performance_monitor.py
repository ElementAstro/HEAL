...
from app.components.module.performance_monitor import (
...
class TestSystemResourceMonitor(unittest.TestCase):
    ...
    def test_monitor_creation(self, mock_process_class):
        """测试监控器创建"""
        mock_process = Mock()
    ...
    @patch('psutil.Process')
    def test_get_thread_count(self, mock_process_class):
        """测试获取线程数"""
        mock_process = Mock()
...
class TestModulePerformanceTracker(unittest.TestCase):
    ...
    def test_track_module_operation(self):
        """测试跟踪模块操作"""
    ...
    def test_track_module_error(self):
        """测试跟踪模块错误"""
        # 先进行一些操作
        self.tracker.track_module_operation("test_module", "load")
    ...
    def test_track_module_load_context(self):
        """测试模块加载上下文"""

                ...
                try:
                    ...
                    for item in data:
                        if isinstance(item, dict):
                            issues.append(
                                f"{item.get('path', 'unknown')}:{item.get('line', 0)} - {item.get('message', 'Unknown issue')}"
                            )
    ...
    def check(self, project_path: str) -> QualityResult:
        ...
        try:
            ...
            if result.stdout:
                issues = result.stdout.strip().split("\n")
                issues = [
                    issue for issue in issues if issue.strip() and "error:" in issue
                ]
    ...
    def check(self, project_path: str) -> QualityResult:
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    for issue in data.get("results", []):
                        ...
                        issues.append(
                            f"{filename}:{line_number} - {severity}/{confidence}: {text}"
                        )
            ...
            high_severity_issues = [
                issue for issue in issues if "HIGH/" in issue or "MEDIUM/" in issue
            ]
        ...
        return QualityResult(
            ...
            details={
                ...
                "high_severity": len([i for i in issues if "HIGH/" in i]),
            },
            execution_time=time.time() - start_time,
        )
            ...
            if os.path.exists(coverage_file):
                try:
                    with open(coverage_file, "r") as f:
                        ...
                        details["coverage"] = coverage_data.get("totals", {}).get(
                            "percent_covered", 0
                        )
                except Exception as e:
...
class DependencyChecker(QualityChecker):
    ...
    def is_available(self) -> bool:
        try:
            subprocess.run(
                [sys.executable, "-m", "pip", "check"], capture_output=True, check=True
            )
            return True
        except (subprocess.CalledProcessError, FileNotFoundError):
            return False
    ...
    def check(self, project_path: str) -> QualityResult:
        ...
        try:
            ...
            passed = (
                len([issue for issue in issues if "conflict" in issue.lower()]) == 0
            )
...
class QualityAssurancePipeline:
    ...
    def generate_report(
        self, results: Dict[str, QualityResult], output_file: Optional[str] = None
    ) -> str:
        ...
        for name, result in results.items():
            status = "✓ PASSED" if result.passed else "✗ FAILED"
            score_str = (
                f" ({result.score:.1f}/10.0)" if result.score is not None else ""
            )
    ...
    def export_json_report(
        self, results: Dict[str, QualityResult], output_file: str
    ) -> None:
        ...
        json_data = {
            ...
            "summary": {
                "total_checks": len(results),
                "passed_checks": sum(1 for r in results.values() if r.passed),
                "success_rate": sum(1 for r in results.values() if r.passed)
                / len(results)
                * 100,
            },
            "results": {},
